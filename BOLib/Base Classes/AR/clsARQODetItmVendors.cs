...
{
    /// <summary>
    /// Summary description for ARQODetItmVendors.
    /// </summary>
    [Serializable]
    public class ARQODetItmVendors : DataTable
    {
        #region +++  Constructor  +++

        public ARQODetItmVendors()
        {
        ...
        public ARQODetItmVendors(SqlConnection cn)
        {
        ...
        internal bool Save(SqlConnection cn, int? headerKey, DataTable dt)
        {
            using (StringWriter swStringWriter = new StringWriter())
            {
                ...
                using (SqlCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandType = CommandType.StoredProcedure;
                    cm.CommandText = "ARQO_DetItm_Vendor_Save";

                    // Datatable as XML format 
                    dt.TableName = "AR_QODetItm_Vendor";
                    DataSet ds = new DataSet();
                    ds.Tables.Add(dt);

                    //Change Column mapping type to Attribute
                    foreach (DataColumn dc in ds.Tables["AR_QODetItm_Vendor"].Columns)
                    {
        ...
        internal bool Insert(SqlConnection cn, out string msgID)
        {
            ...
            foreach (DataRow dr in this.Rows)
            {
                using (SqlCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.Parameters.AddWithValue("@VendorEqItmQty", dr["VendorEqItmQty"]);
                    cm.Parameters.AddWithValue("@VendorEqItmPrice", dr["VendorEqItmPrice"]);
                    cm.Parameters.AddWithValue("@VendorItmQty", dr["VendorItmQty"]);
                    cm.Parameters.AddWithValue("@VendorItmPrice", dr["VendorItmPrice"]);
                    cm.Parameters.AddWithValue("@VendorLeadTime", dr["VendorLeadTime"]);
                    cm.Parameters.AddWithValue("@VendorWarranty", dr["VendorWarranty"]);
                    cm.Parameters.AddWithValue("@VendorRemarks", dr["VendorRemarks"]);
                    cm.Parameters.AddWithValue("@Selected", dr["Selected"]);
                    cm.Parameters.AddWithValue("@VendorStatus", dr["VendorStatus"]);
