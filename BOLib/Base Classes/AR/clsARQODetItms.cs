...
{
    /// <summary>
    /// Summary description for ARQODetItms.
    /// </summary>
    [Serializable]
    public class ARQODetItms : DataTable
    {
        #region +++  Constructor  +++
        ...
        public ARQODetItms()
        {
            ...
            if (!(GFunc.CompareString(stack.GetMethod().Name, "Copy") || GFunc.CompareString(stack.GetMethod().Name, "Clone")) && !GFunc.CompareString(stack.GetMethod().Name, "Clone"))
                this.Fetch(new Criteria(0, 1));
        }
        public ARQODetItms(SqlConnection cn)
        {
        ...
        internal bool Save(SqlConnection cn, int? headerKey, DataTable dt)
        {
            using (StringWriter swStringWriter = new StringWriter())
            {
                ...
                using (SqlCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandType = CommandType.StoredProcedure;
                    cm.CommandText = "ARQO_DetItm_Save";

                    // Datatable as XML format 
                    dt.TableName = "AR_QODetItm";
                    DataSet ds = new DataSet();
                    ds.Tables.Add(dt);

                    //Change Column mapping type to Attribute
                    foreach (DataColumn dc in ds.Tables["AR_QODetItm"].Columns)
                    {
                    ...
                    parameter.ParameterName = "@xmlAR_QODetItm";
        ...
        internal bool Update(SqlConnection cn, out string msgID)
        {
            ...
            foreach (DataRow dr in this.Rows)
            {
                using (SqlCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.Parameters.AddWithValue("@ARSOID", dr["ARSOID"]);
                    cm.Parameters.AddWithValue("@ARSODK", dr["ARSODK"]);
                    cm.Parameters.AddWithValue("@ARSODItm", dr["ARSODItm"]);
                    cm.Parameters.AddWithValue("@ARSOPOID", dr["ARSOPOID"]);
