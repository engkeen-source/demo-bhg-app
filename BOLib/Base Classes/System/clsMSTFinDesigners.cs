...
{
    [Serializable()]
    public class MSTFinDesigners : DataTable
    {

        #region Factory Methods

        public MSTFinDesigners()
        {
            this.Fetch( new Criteria(-1, 1));
        }        

        public MSTFinDesigners(SqlConnection cn)
        {
            this.Fetch(cn, new Criteria(-1, 1));
        }

        public new MSTFinDesigners Clone()
        {
             MSTFinDesigners objCopy = (MSTFinDesigners)this.MemberwiseClone();
            return objCopy;
        }

        public static MSTFinDesigners Get( int? headerKey)
        {
            MSTFinDesigners obj = new MSTFinDesigners();
            obj.Fetch(new Criteria(headerKey, 1));
            return obj;
        }

        public static MSTFinDesigners New()
        {
            MSTFinDesigners obj = new MSTFinDesigners();
            return obj;
        }
        public static MSTFinDesigners New(SqlConnection cn)
        {
            MSTFinDesigners obj = new MSTFinDesigners();
        ...
        internal class Criteria
        {
            ...
            internal Criteria(int? RepKey, int? RepDetKey,int? ColNo, int? Option)
            {
                _repDetKey = RepDetKey;
                _colNo = ColNo;
                _repKey = RepKey;
        ...
        try
        {
            using (StringWriter swStringWriter = new StringWriter())
            {
                ...
                using (SqlCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandType = CommandType.StoredProcedure;
                    cm.CommandText = "MST_FinDesigner_Save";

                    // Datatable as XML format 
                    string strMST_FinDesigners = GFunc.ConvertDataTableToXML(this);
                    // Add input parameter and set its properties.
                    SqlParameter parameter = new SqlParameter();
                    // Store procedure parameter name  
                    parameter.ParameterName = "@xmlMST_FinDesigner";
                    // Parameter type as XML 
                    parameter.DbType = DbType.Xml;
                    parameter.Direction = ParameterDirection.Input; // Input Parameter  
                    parameter.Value = strMST_FinDesigners; // XML string as parameter value  
            ...
            try
            {
                ...
                foreach (DataRow dr in this.Rows)
                {
                    ...
                    using (SqlCommand cm = cn.CreateCommand())
                    {
                        ...
                        cm.Parameters.AddWithValue("@DesignerText", dr["DesignerText"]);
            ...
            try
            {
                foreach (DataRow dr in this.Rows)
                {
                    using (SqlCommand cm = cn.CreateCommand())
                    {
                        ...
                        cm.Parameters.AddWithValue("@LastModifiedDate", dr["LastModifiedDate"].ToString() == "" ? DateTime.Now : GFunc.NEDateTime(dr["LastModifiedDate"],0));
