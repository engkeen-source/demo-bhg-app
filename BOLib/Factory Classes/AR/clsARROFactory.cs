...
{
    [Serializable()]
    public class ARROFactory
    {
        #region Member variables and constants
        private ARRO _Doc = null;
        private ARRODetItms _DocDetItms = null;

        private GEnum.InstanceMode _instanceMode = GEnum.InstanceMode.Normal;
        private int _guID = 0;
        private bool _isError = false;
        private GEnum.SystemCode _codeKey = GEnum.SystemCode.Reserve_Order;
        ...
        public ARROFactory(GEnum.InstanceMode instanceMode, GEnum.SystemCode DocCodeKey)
        {
            try
            {
                ...
                if (this.InstanceMode == GEnum.InstanceMode.Normal)
                {
                    ...
                    using (TransactionScope scope = new TransactionScope())
                    {
                        using (SqlConnection cn = new SqlConnection(Database.BossDemoConnection))
                        {
                            ...
                            this._DocDetItms = new ARRODetItms(cn);
                            #endregion
        ...
        //Doc Methods
        public string New(TAUtil.TAGridEditor grd)
        {
            #region Declaration
            ARRO copyARRO = null;
            ARRODetItms copyARRODetItms = null;
            ...
            try
            {
                ...
                if (_DocDetItms != null)
                    copyARRODetItms = GFunc.TACopyDataTable(_DocDetItms);
                #endregion
            ...
            finally
            {
                ...
                {
                    this._Doc = copyARRO;
                    this._DocDetItms = copyARRODetItms;
                }
                #endregion

                #region Reset Backup Objects
                copyARRO = null;
                copyARRODetItms = null;
                #endregion
            }
        }//Completed
            ...
            try
            {
                ...
                using (TransactionScope scope = new TransactionScope())
                {
                    using (SqlConnection cn = new SqlConnection(Database.BossDemoConnection))
                    {
                        ...
                        //By DocKey
                        if (_Doc.Fetch(cn, new ARRO.Criteria((int)_codeKey, docKey, 1)) == false)
                        {
                        ...
                        _DocDetItms.Fetch(cn, new ARRODetItms.Criteria(docKey, 1));
                ...
                _Doc.IsReadOnly = _Doc.DocState == (int)GEnum.DocState.Pending; /* added by YST on 2022/06/30 */
        ...
        public string Delete(TAUtil.TAGridEditor grd)
        {
            ...
            try
            {
                ...
                using (SqlConnection cn = new SqlConnection(Database.BossDemoConnection))
                {
                    ...
                    _Doc = new ARRO();
                    _Doc._GUID = _guID;
                    _DocDetItms = new ARRODetItms(cn);
                }
                this.New(grd);
                return GVar.gcPass;
            }
            ...
            try
            {
                ...
                using (TransactionScope scope = new TransactionScope())
                {
                    using (SqlConnection cn = new SqlConnection(Database.BossDemoConnection))
                    {
                        ...
                        {
                            _Doc._DocReqDate = null;/* added by YST on 2022/10/09 requested by Feliani,Patrick */
                            if (_Doc.DocTypeNm.ToLower().Contains("estore")) /* added by YST on 2025/02/24 */
                            {
                                _Doc.DocRef = string.Empty; // estore_reference_no
                                _Doc.Custom2 = string.Empty; // estore_reference_id
                                _Doc.Custom3 = string.Empty; // reply_status
                            }
                        }
                        ...
                        foreach (DataRow item in _DocDetItms.Rows)
                        {
                            ...
                            item["NSLink"] = "11151-" + _Doc._DocKey + "-" + GFunc.NEStr(item["DocItmKey"], "");                          
                            item["ItmQtyLink"] = 0;
                            item["ItmQtyAdj"] = 0;
                            item["ItmQtyDelivered"] = 0;
                        ...
                        _Doc.Attachments.Clear();//Added by May on 13-Apr-2025
                        #endregion
        ...
        public string CopyFrom(GEnum.SystemCode sourceDocCodekey, int sourceDocKey, TAUtil.TAGridEditor grd, bool NSLink, out DataTable dtDetail)
        {
            DataSet dsCopy = null;
                    ...
                    using (SqlConnection cn = new SqlConnection(Database.BossDemoConnection))
                    {
                        ...
                        switch (source_DC)
                        {
                            case (int)GEnum.SystemCode.Quotation:
                                if (!_Doc.Fetch_ARQO(cn, new ARRO.Criteria((int)CodeKey, source_DK, _Doc.DocKey)))
                                {
                                ...
                                _DocDetItms.Clear();
                                _DocDetItms.Fetch_ARQODetItm(cn, new ARRODetItms.Criteria(source_DK, (int)CodeKey, _Doc.DocKey));

                                //// _Doc.Attachments.CopyWithDMAS(cn, source_DC, source_DK, (int)CodeKey,GFunc.NEInt(_Doc.DocKey,0));  /* Commented by KKAung on 22 Feb 2023,  Do not copy QO's attachments to RO */
                                //_Doc.Attachments = new SYSAttachments();
                                //// Added by KKAung on 22 Feb 2023
                                ///

                                _Doc.Attachments.CopyWithDMAS(cn, source_DC, source_DK, (int)CodeKey, GFunc.NEInt(_Doc.DocKey, 0)); /* to copy attachments from QO to RO */
        ...
        private bool Doc_SetDefaultValue(SqlConnection cn)
        {
            try
            {
                ...
                if (SysOptionUtility.DatabaseBranchCode == "BHM") _Doc._DocReqDate = null;/* added by YST on 2022/10/09 requested by Feliani,Patrick */
