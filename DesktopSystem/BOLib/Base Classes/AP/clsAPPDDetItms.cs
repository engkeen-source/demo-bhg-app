...
{
    /// <summary>
    /// Summary description for APPDDetItms.
    /// </summary>
    [Serializable]
    public class APPDDetItms : DataTable
    {
        #region +++  Constructor  +++
        ...
        public APPDDetItms(SqlConnection cn)
        {
        ...
        internal bool Fetch_APPODetItm(SqlConnection cn, Criteria criteria)
        {
            using (SqlCommand cm = cn.CreateCommand())
            {
                cm.CommandType = CommandType.StoredProcedure;
                cm.CommandText = "APPODetItm_CreateDoc";
                int retVal = 0;
                cm.Parameters.AddWithValue("@DocKey", criteria._DocKey);
                ...
                cm.Parameters.AddWithValue("@RetValue", retVal);
                cm.Parameters["@RetValue"].Direction = ParameterDirection.Output;
        ...
        internal bool Fetch_APPSDetItm(SqlConnection cn, Criteria criteria)
        {
            using (SqlCommand cm = cn.CreateCommand())
            {
                cm.CommandType = CommandType.StoredProcedure;
                cm.CommandText = "APPSDetItm_CreateDoc";
                int retVal = 0;
                cm.Parameters.AddWithValue("@DocKey", criteria._DocKey);
                cm.Parameters.AddWithValue("@DocCodeKey", criteria._DocCodeKey);
                cm.Parameters.AddWithValue("@NewDocKey", criteria._NewDocKey);
        ...
        internal bool Save(SqlConnection cn, int? headerKey, DataTable dt)
        {
            using (StringWriter swStringWriter = new StringWriter())
            {
                ...
                using (SqlCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandType = CommandType.StoredProcedure;
                    cm.CommandText = "AP_PDDetItm_Save";

                    // Datatable as XML format 
                    dt.TableName = "AP_PDDetItm";
                    DataSet ds = new DataSet();
                    ds.Tables.Add(dt);

                    //Change Column mapping type to Attribute
                    foreach (DataColumn dc in ds.Tables["AP_PDDetItm"].Columns)
                    {
        ...
        internal bool Delete(SqlConnection cn, Criteria criteria, out string msgID)
        {
            ...
            {
                cm.CommandType = CommandType.StoredProcedure;
                cm.CommandText = "APPDDetItm_Delete";
