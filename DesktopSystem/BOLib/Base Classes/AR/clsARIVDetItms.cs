...
{
    /// <summary>
    /// Summary description for ARIVDetItms.
    /// </summary>
    [Serializable]
    public class ARIVDetItms : DataTable
    {
        #region +++  Constructor  +++
        ...
        public ARIVDetItms(SqlConnection cn)
        {
        ...
        #region Data Access - Other Det Fetch  From other applied Docs
        ...
        internal bool Save(SqlConnection cn, int? headerKey, DataTable dt)
        {
            using (StringWriter swStringWriter = new StringWriter())
            {
                ...
                using (SqlCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandType = CommandType.StoredProcedure;
                    cm.CommandText = "AR_IVDetItm_Save";

                    // Datatable as XML format 
                    dt.TableName = "AR_IVDetItm";
                    DataSet ds = new DataSet();
                    ds.Tables.Add(dt);

                    //Change Column mapping type to Attribute
                    foreach (DataColumn dc in ds.Tables["AR_IVDetItm"].Columns)
                    {
        ...
        internal bool Delete(SqlConnection cn, Criteria criteria, out string msgID)
        {
            ...
            {
                cm.CommandType = CommandType.StoredProcedure;
                cm.CommandText = "ARIVDetItm_Delete";
