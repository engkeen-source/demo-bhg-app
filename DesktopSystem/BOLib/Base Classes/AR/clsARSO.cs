...
{
	/// <summary>
	/// Summary description for ARSO.
	/// </summary>
	[Serializable]
    public class ARSO : Document, INotifyPropertyChanged
    {
            ...
            : base()
        {
            ...
            base.PropertyChanged += new PropertyChangedEventHandler(ARSO_PropertyChanged);
        }
        void ARSO_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
        ...
        public ARSO Clone()
        {
            ARSO objCopy = (ARSO)this.MemberwiseClone();
            return objCopy;
        }
        ...
        public static ARSO Get(int? docKey)
        {
            ARSO child = new ARSO();
        ...
        internal static ARSO New(out string msgID)
        {
        ...
        internal bool Fetch_ARRO(SqlConnection cn, Criteria criteria)
        {
            bool retValue = false;
            using (SqlCommand cm = cn.CreateCommand())
            {
                cm.CommandType = CommandType.StoredProcedure;
                cm.CommandText = "ARRO_CreateDoc";
		...
		internal static ARSO Get(IDataReader dr)
		{
			ARSO child = new ARSO();
			child.Fetch(dr);
			return child;
		}
        internal static ARSO Get(SqlConnection cn, Criteria criteria)
        {
            ARSO child = new ARSO();
		...
		{			
			...
			using(SqlCommand cm = cn.CreateCommand())
			{
                ...
                if (_PrintDept == null)
                    cm.Parameters.AddWithValue("@PrintDept", DBNull.Value);
                else
                    cm.Parameters.AddWithValue("@PrintDept", _PrintDept);
		...
		internal bool Validation(SqlConnection cn, Criteria criteria, out string msgID,bool isNew)
		{
			...
			{
				cm.CommandType = CommandType.StoredProcedure;
				cm.CommandText = "ARSO_Validation";
