...
{
    /// <summary>
    /// Summary description for INMFNDetItms.
    /// </summary>
    [Serializable]
    public class INMFNDetItms : DataTable
    {
        #region +++  Constructor  +++
        ...
        public INMFNDetItms(SqlConnection cn)
        {
        ...
        internal bool Fetch(SqlConnection cn, Criteria criteria)
        {
            using (SqlCommand cm = cn.CreateCommand())
            {
                ...
                cm.CommandText = "INMFNDetItm_Get";
        ...
        internal bool Save(SqlConnection cn, int? headerKey, DataTable dt)
        {
            using (StringWriter swStringWriter = new StringWriter())
            {
                ...
                using (SqlCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandType = CommandType.StoredProcedure;
                    cm.CommandText = "IN_MFNDetItm_Save";

                    // Datatable as XML format 
                    dt.TableName = "IN_MFNDetItm";
                    DataSet ds = new DataSet();
                    ds.Tables.Add(dt);

                    //Change Column mapping type to Attribute
                    foreach (DataColumn dc in ds.Tables["IN_MFNDetItm"].Columns)
                    {
        ...
        internal bool Delete(SqlConnection cn, Criteria criteria, out string msgID)
        {
            ...
            {
                cm.CommandType = CommandType.StoredProcedure;
                cm.CommandText = "INMFNDetItm_Delete";
