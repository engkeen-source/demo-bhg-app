...
{
    [Serializable()]
    public class REFTaxADetItms : DataTable
    {

        #region Factory Methods

        public REFTaxADetItms()
        {
            this.Fetch(new Criteria(-1,DateTime.Today,1));
        }
        public REFTaxADetItms(SqlConnection cn)
        {
            this.Fetch(cn, new Criteria(-1,DateTime.Today,1));
        }
        public static REFTaxADetItms New()
        {
            REFTaxADetItms obj = new REFTaxADetItms();
            return obj;
        }

        internal static REFTaxADetItms Get()
        {
            REFTaxADetItms obj = new REFTaxADetItms();
            obj.Fetch(new Criteria(0, DateTime.Today, 0));
            return obj;
        }

        internal static REFTaxADetItms Get( int? taxKey)
        {
            REFTaxADetItms obj = new REFTaxADetItms();
            obj.Fetch(new Criteria(taxKey, DateTime.Today, 1));
            return obj;
        }
        ...
        internal bool Insert(SqlConnection cn, int? headerKey)
        {
            ...
            foreach (DataRow dr in this.Rows)
            {
                ...
                using (SqlCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.Parameters.AddWithValue("@TaxKey", headerKey);
                    cm.Parameters.AddWithValue("@EffDate", dr["EffDate"]);
                    cm.Parameters.AddWithValue("@TaxRate", dr["TaxRate"]);
