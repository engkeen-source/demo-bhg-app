...
{
    /// <summary>
    /// Summary description for SYSRepCriterias.
    /// </summary>
    [Serializable]
    public class SYSRepCriterias : DataTable
    {
        #region +++  Constructor  +++

        public SYSRepCriterias()
        {
            SYSRepCriteria obj = new SYSRepCriteria();

            this.Fetch(new Criteria(0, 1));

        }
        public SYSRepCriterias(SqlConnection cn)
        {
            string msgID = string.Empty;
            this.Fetch(cn, new Criteria(0, 1));
        }       

        public static SYSRepCriterias Get(int RepKey)
        {
            SqlConnection cn = new SqlConnection(Database.BossDemoConnection);
            SYSRepCriterias obj = new SYSRepCriterias();
            obj.Fetch(cn, new Criteria(RepKey));
            return obj;
        }

        public static SYSRepCriterias Get(SqlConnection cn,int RepKey)
        {
            SYSRepCriterias obj = new SYSRepCriterias();
            obj.Fetch(cn, new Criteria(RepKey));
            return obj;
        ...
        internal bool Fetch(SqlConnection cn, Criteria criteria)
        {
            using (SqlCommand cm = cn.CreateCommand())
            {
                ...
                cm.Parameters.AddWithValue("@RepKey", criteria._DocKey);
        ...
        internal bool Save(SqlConnection cn, int? headerKey, DataTable dt)
        {
            using (StringWriter swStringWriter = new StringWriter())
            {
                ...
                using (SqlCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandType = CommandType.StoredProcedure;
                    cm.CommandText = "SYS_RepCriteria_Save";

                    // Datatable as XML format 
                    dt.TableName = "SYS_RepCriteria";
                    DataSet ds = new DataSet();
                    ds.Tables.Add(dt);

                    //Change Column mapping type to Attribute
                    foreach (DataColumn dc in ds.Tables["SYS_RepCriteria"].Columns)
                    {
