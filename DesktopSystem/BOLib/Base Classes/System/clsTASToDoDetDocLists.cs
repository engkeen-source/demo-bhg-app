...
{
    /// <summary>
    /// Summary description for TASToDoDetDocLists.
    /// </summary>
    [Serializable]
    public class TASToDoDetDocLists : DataTable
    {
        #region +++  Constructor  +++

        public TASToDoDetDocLists()
        {
            this.Fetch(new Criteria(0, 1));

        }
        public TASToDoDetDocLists(SqlConnection cn)
        {
        ...
        internal bool Fetch(SqlConnection cn, Criteria criteria)
        {
            using (SqlCommand cm = cn.CreateCommand())
            {
                ...
                cm.CommandText = "TASToDoDetDocList_Get";
        ...
        internal bool Save(SqlConnection cn, int? headerKey, DataTable dt)
        {
            using (StringWriter swStringWriter = new StringWriter())
            {
                ...
                using (SqlCommand cm = cn.CreateCommand())
                {
                    ...
                    cm.CommandType = CommandType.StoredProcedure;
                    cm.CommandText = "TASToDoDetDocList_Save";
                    TASToDoDetDocLists dtNew = new TASToDoDetDocLists(cn);
                    GFunc.CopyDocumentDetail(dt, dtNew);
                    // Datatable as XML format 
                    dtNew.TableName = "TAS_ToDoDetDocList";
                    DataSet ds = new DataSet();
                    ds.Tables.Add(dtNew);

                    //Change Column mapping type to Attribute
                    foreach (DataColumn dc in ds.Tables["TAS_ToDoDetDocList"].Columns)
                    {
                    ...
                    string strTAS_ToDoDetDocLists = swStringWriter.ToString();

                    // Add input parameter and set its properties.
                    SqlParameter parameter = new SqlParameter();
                    // Store procedure parameter name  
                    parameter.ParameterName = "@xmlTAS_ToDoDetDocList";
                    // Parameter type as XML 
                    parameter.DbType = DbType.Xml;
                    parameter.Direction = ParameterDirection.Input; // Input Parameter  
                    parameter.Value = strTAS_ToDoDetDocLists; // XML string as parameter value  
        ...
        internal bool Delete(SqlConnection cn, Criteria criteria)
        {
            ...
            {
                cm.CommandType = CommandType.StoredProcedure;
                cm.CommandText = "TASToDoDetDocList_Delete";
                cm.Parameters.AddWithValue("@ToDoKey", criteria._ToDoKey);
