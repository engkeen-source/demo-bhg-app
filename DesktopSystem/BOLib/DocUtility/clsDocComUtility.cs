...
namespace BOLib
{
    public class DocComUtility
    {
        ...
        public static bool CalForm(Document objDoc, Hashtable details, bool CalTax, bool RunCheck, bool ColTaxAmt = false, bool POCsgItems = false)
        {
            try
            {
                ...
                {
                    cn.Open();
                    return CalForm(cn, objDoc, details, CalTax, RunCheck, ColTaxAmt, POCsgItems);
                }
                //else
                //    return true;
            }
        ...
        public static bool CalForm(SqlConnection cn, Document objDoc, Hashtable details, bool CalTax, bool RunCheck, bool ColTaxAmt = false, bool POCsgItems = false)
        {
            ...
            try
            {
                ...
                switch (objDoc.DocCodeKey)
                {
                    ...
                    case (int)GEnum.SystemCode.Purchase_Order:
                        if (POCsgItems)
                            DocComUtility.DocDetail_Get(GEnum.Details.Doc_CsgItm, details, ref dtItem);
                        else
                            DocComUtility.DocDetail_Get(GEnum.Details.Doc_Itm, details, ref dtItem);
                        break;
                    case (int)GEnum.SystemCode.Packing_List:
                ...
                switch (objDoc.DocCodeKey)
                {
                    ...
                    case (int)GEnum.SystemCode.Cash_Sale:
                    case (int)GEnum.SystemCode.Cash_Debit_Note:
                        return CalDocAR(cn, objDoc, dtItem, CalTax, RunCheck, ColTaxAmt);

                    case (int)GEnum.SystemCode.Sales_Credit_Note:
                    case (int)GEnum.SystemCode.Cash_Credit_Note:
                        if (CalDocAR(cn, objDoc, dtItem, CalTax, RunCheck, ColTaxAmt))
                            if (RunCheck == false)
                    ...
                    case (int)GEnum.SystemCode.Purchase_Invoice:
                    case (int)GEnum.SystemCode.Purchase_Debit_Note:
                        return CalDocAP(cn, objDoc, dtItem, CalTax, RunCheck, ColTaxAmt, POCsgItems);
                    ...
                    case (int)GEnum.SystemCode.Purchase_Credit_Note:
                        if (CalDocAP(cn, objDoc, dtItem, CalTax, RunCheck, ColTaxAmt))
                            if (RunCheck == false)
                                return ApplyIV_Reset(objDoc);
                            else
                                return true;
                    ...
                    case (int)GEnum.SystemCode.Cash_Payment_Received:
                    case (int)GEnum.SystemCode.Payment_Issue:
                        return CalDocPY(cn, objDoc, dtExp, dtItem, CalTax, RunCheck, ColTaxAmt);
        ...
        private static bool CalDocAR(SqlConnection cn, Document objDoc, DataTable dtItems, bool CalTax, bool RunCheck, bool ColTaxAmt = false)
        {
            #region variable
            IEnumerable<DataRow> dtItemFilter = null;
            //added by thettm on 28-jan-2019(start)
            IEnumerable<DataRow> dtItemProcessing = null;
            //added by thettm on 28-jan-2019(end)
            ...
            /* added by YST on 2019/11/04 */
            decimal? TotalHomeSub = 0;//Home Sub Total
            decimal? TotalHomeTax = 0;//Home Tax Total
            decimal? TotalHomeAfterDis = 0;//Home Tax Total
            ...
            try
            {
                ...
                if (objDoc.DocCodeKey == (int?)GEnum.SystemCode.Sales_Order)
                    DocCompleted =GFunc.NEBool(GFunc.GetPropertyValue("DocCompleted", objDoc),false);
                    ...
                    foreach (DataRow row in dtItemFilter)
                    {
                        ...
                        switch (GFunc.GetINTypeGroup(GFunc.NEInt(row["ItmType"], 0)))
                        {
                            ...
                            case (int)GEnum.INTypeGrp.Non_Stock:
                                ...
                                {
                                    #region commented by YST on 2023/10/24 to allow to key different ColumnTaxAmount as Purchase Invoice
                                    /*
                                    ...
                                    */
                                    ...
                                    /* added by by YST on 2023/10/24 to allow ColumnTaxAmount to follow Purchase Invoice that requested by Josie */
                                    if ((ItmTaxGrpRate > 0 && DocTaxKey != ItmTaxGrpKey) == true)
                                        ColTaxAmt = true;
                                    ...
                                    if (ItmTaxGrpRate > 0)
                                    {
                                        if (CalTax)
                                        {
                                            if (ColTaxAmt == true)/* added by YST 2019/11/18 to allow to edit Tax Amt */
                                            {
                                                ItmTaxGrpAmtF = GFunc.RndC(GFunc.NEDec(row["ItmTaxGrpAmtF"], 0), GVar.RndDecs.Amtpt);
                                ...
                                #region Update ItmQtyAdj for DocCompleted
                                //added by May on 18-Nov-2020
                                if (objDoc.DocCodeKey == (int?)GEnum.SystemCode.Sales_Order) 
                                {
                                    if (DocCompleted)
                                    {
                                        row["ItmQtyAdj"] = GFunc.NEDec(row["ItmQty"], 0) - GFunc.NEDec(row["ItmQtyLink"], 0);
                                    }
                                    else
                                    {
                                        row["ItmQtyAdj"] = 0;
                                    }
                                    row["ItmQtyBalance"] = GFunc.NEDec(row["ItmQty"], 0) - GFunc.NEDec(row["ItmQtyLink"], 0) - GFunc.NEDec(row["ItmQtyAdj"], 0);
                                    row["ItmQtyDelivered"] = GFunc.NEDec(row["ItmQtyLink"], 0) + GFunc.NEDec(row["ItmQtyAdj"], 0);
                                }
                                #endregion
                            ...
                            case (int)GEnum.INTypeGrp.Charges:
                                ...
                                if (row["ItmID"].ToString().ToLower().Contains("bank")) /* added by YST on 2021/11/05, suggested by Josie from finance  */
                                {
                                    //need to set ItmTax to null when Item ID is Bank Charges
                                    ItmTaxGrpKey = 0;
                                    ItmTaxGrpRate = 0;
                                    ItmTaxGrpAmtF = 0;
                                    ItmTaxGrpAmtL = 0;
                                ...
                                else if (DocTaxRate == 0 && ItmTaxGrpRate > 0)
                                {
                                    //need to set ItmTax to DocTax when doc DocTax is 0% and ItmTax is <> 0%
                                    ItmTaxGrpKey = DocTaxKey;
                                    ItmTaxGrpRate = 0;
                                ...
                                else
                                {
                                    ...
                                    */
                                    ...
                                    if (ItmTaxGrpRate > 0)
                                    {
                                        if (CalTax)
                                        {
                                            if (ColTaxAmt == true)/* added by YST 2019/11/18 to allow to change Tax Amt */
                                            {
                                                ItmTaxGrpAmtF = GFunc.RndC(GFunc.NEDec(row["ItmTaxGrpAmtF"], 0), GVar.RndDecs.Amtpt);
                                                ItmTaxGrpAmtL = GFunc.RndC(ItmTaxGrpAmtF * DocCountryRate, GVar.RndDecs.Amtpt);
                                            }
                ...
                if (RunCheck)
                {
                    ...
                    /* added by YST on 2019/11/04 */
                    TotalHomeSub = GFunc.RndC(DetTotalAmtF * DocCurrRate, GVar.RndDecs.Amtpt);
                    TotalHomeTax = GFunc.RndC(DocTaxAmt * DocCurrRate, GVar.RndDecs.Amtpt);
                    TotalHomeAfterDis = GFunc.RndC(DocOverallDisAmt * DocCurrRate, GVar.RndDecs.Amtpt);
                    ...
                    //if (GFunc.NEDec(GFunc.GetPropertyValue("DocHome", objDoc), 0) != GFunc.RndC(GFunc.GetDecimalPropertyValue("DocGrand", objDoc) * DocCurrRate, GVar.RndDecs.Amtpt)) /* commented by YST on 20191118 */
                    if (GFunc.NEDec(GFunc.GetPropertyValue("DocHome", objDoc), 0) != GFunc.RndC(TotalHomeSub - TotalHomeAfterDis + TotalHomeTax, GVar.RndDecs.Amtpt)) /* added by YST on 20191118 to change Home Total Logic */
                        FailCheck = true;
                ...
                else
                {
                    ...
                    if (CalTax)
                    {
                        if (SysOptionUtility.TaxCalculationOnTotal && !ColTaxAmt)
                        {
                            TotalTaxableAmtF = TotalTaxableAmtF - GFunc.RndDC(TotalTaxableAmtF * DocDisRate, 100M, GVar.RndDecs.Amtpt);
                            DocTaxAmt = GFunc.RndC(TotalTaxableAmtF * DocTaxRate, GVar.RndDecs.Amtpt);
                            DocTaxAmtL = GFunc.RndC(TotalTaxableAmtF * DocTaxRate * DocCountryRate, GVar.RndDecs.Amtpt);
                    ...
                    GFunc.SetPropertyValue("DocHomeSubTotal", objDoc, TotalHomeSub);
                    GFunc.SetPropertyValue("DocHomeTaxTotal", objDoc, TotalHomeTax);
                    GFunc.SetPropertyValue("DocHome", objDoc, GFunc.RndC(TotalHomeSub - TotalHomeAfterDis + TotalHomeTax, GVar.RndDecs.Amtpt)); /* added by YST on 20191118 to change Home Total Logic */
                    //GFunc.SetPropertyValue("DocHome", objDoc, GFunc.RndC(GFunc.GetDecimalPropertyValue("DocGrand", objDoc) * DocCurrRate, GVar.RndDecs.Amtpt)); /* commented by YST on 20191118 */
                    GFunc.SetPropertyValue("DocTaxTotalLocal", objDoc, DocTaxAmtL);
                    #endregion
                }
                //added by thettm on 28-jan-2019(start)
                #region CalculateProcessingFee
                if (dtItems.Rows.Count > 0 && RunCheck == true)
                {
                    dtItemProcessing = dtItems.AsEnumerable().Where(r => r.Field<int>("ItmKey") == SysOptionUtility.ProcessingItem);
                    if (dtItemProcessing.Count() > 0 && objDoc.DocTypeNm.ToUpper().Contains("ESTORE"))
                    {

                        foreach (DataRow row in dtItemProcessing)
                        {
                            decimal subTotal = GFunc.NEDec(GFunc.GetPropertyValue("DocSubTotal", objDoc), 0) - GFunc.NEDec(row["ItmAmtF"].ToString(), 0);
                            decimal gst = GFunc.RndC(subTotal * GFunc.NEDec(GFunc.GetPropertyValue("DocTaxGrpRate", objDoc), 0), 2);
                            decimal fee = GFunc.RndC((subTotal + gst) * SysOptionUtility.ProcessingFee /*0.047M*/, 2);
                            if (GFunc.NEDec(row["ItmAmtF"].ToString(), 0) != fee)
                            {
                                row["ItmPriceBefore"] = fee;
                                row["ItmPriceAfter"] = fee;
                                row["ItmPrice"] = fee;
                                row["ItmAmtF"] = fee;
                                row["ItmAmtH"] = GFunc.RndC(fee * GFunc.NEDec(GFunc.GetPropertyValue("DocCurrRate", objDoc), 0), 2);
                                row["ItmAmtShw"] = fee;
                                row["ItmPriceUser"] = fee;
                                CalDocAR(cn, objDoc, dtItems, CalTax, false);
                                return CalDocAR(cn, objDoc, dtItems, CalTax, true);
                            }
                        }
                    }
                }
        ...
        private static bool CalDocAP(SqlConnection cn, Document objDoc, DataTable dtItems, bool CalTax, bool RunCheck, bool ColTaxAmt = false, bool POCsgItems = false)
        {
            ...
            try
            {
                    ...
                    foreach (DataRow row in dtItemFilter)
                    {
                        ...
                        switch (GFunc.GetINTypeGroup(GFunc.NEInt(row["ItmType"], 0)))
                        {
                            ...
                            case (int)GEnum.INTypeGrp.Non_Stock:
                                ...
                                else
                                {
                                    ...
                                    */
                                    ...
                                    /* added by by YST on 2023/01/18 to allow ColumnTaxAmount */
                                    if ((ItmTaxGrpRate > 0 && DocTaxKey != ItmTaxGrpKey) == true)
                                        ColTaxAmt = true;
                            ...
                            case (int)GEnum.INTypeGrp.Charges:
                                ...
                                CurItmAmt = ItmAmtShw;
                                DocAddCostChargesH = (row["Custom1"].ToString().ToLower() == "distributed charges") ?  DocAddCostChargesH + ItmAmtH : 0; /* added by YST on 2021/08/15 */
                                TotalTaxAmtF = TotalTaxAmtF + ItmTaxGrpAmtF;
                                TotalTaxAmtL = TotalTaxAmtL + ItmTaxGrpAmtL;
                                DetTotalAmtF = DetTotalAmtF + ItmAmtF;
                                if (ItmTaxGrpAmtF != 0)
                ...
                if (RunCheck)
                {
                    ...
                    TotalHomeSub = GFunc.RndC(DetTotalAmtF * DocCurrRate, GVar.RndDecs.Amtpt);
                    //TotalHomeTax = GFunc.RndC(DocTaxAmt * DocCurrRate, GVar.RndDecs.Amtpt);
                    TotalHomeTax = DocTaxAmtL; //May changed on 10-Mar-2026 as per Josie request
                    TotalHomeAfterDis = GFunc.RndC(DocOverallDisAmt * DocCurrRate, GVar.RndDecs.Amtpt);
                ...
                else
                {
                    ...
                    if (POCsgItems)
                    {
                        GFunc.SetPropertyValue("DocCsgTotal", objDoc, DetTotalAmtF);
                    }
                    else
                    {
                        GFunc.SetPropertyValue("DocSubTotal", objDoc, DetTotalAmtF);
                        ...
                        GFunc.SetPropertyValue("DocHomeSubTotal", objDoc, TotalHomeSub);
                        GFunc.SetPropertyValue("DocHomeTaxTotal", objDoc, TotalHomeTax);
                        GFunc.SetPropertyValue("DocHome", objDoc, GFunc.RndC(TotalHomeSub - TotalHomeAfterDis + TotalHomeTax, GVar.RndDecs.Amtpt));
                    ...
                    switch (objDoc.DocCodeKey)
                    {
                        ...
                        case (int)GEnum.SystemCode.Purchase_Debit_Note:
                            ...
                            //comment by jane . this code move to below.
                            ...
                            //        foreach (DataRow row in dtItemFilter)
                            //        {
                            //            DataTable dtt = dtItemFilter.CopyToDataTable();
                            //            switch (GFunc.GetINTypeGroup(GFunc.NEInt(row["ItmType"], 0)))
                            //            {
                            //                case (int)GEnum.INTypeGrp.Stock:
                            //                case (int)GEnum.INTypeGrp.Non_Stock:
                            //                    if (i < dtItemFilter.Count() - 1)
                            //                        nextSN = GFunc.NEDec(dtItemFilter.ElementAt(i + 1)["ItmSN"], 0); //dtloop aldy excluded inbetween Header/Remark rows  
                            //                    if (nextSN == disSN) //Item Row is just before discount row (no total line in between)
                            //                    {
                            //                        row["ItmAddDisAmtH"] = GFunc.NEDec(drDis["ItmAmtH"], 0) * -1;
                            //                        row["ItmAddDisAmtF"] = GFunc.NEDec(drDis["ItmAmtF"], 0) * -1;
                            //                        row["ItmAddDisH"] = GFunc.RndDC(GFunc.NEDec(drDis["ItmAmtH"], 0) * -1, GFunc.NEDec(row["ItmQty"], 0), GVar.RndDecs.Prcpt);
                            //                        row["ItmAddDisF"] = GFunc.RndDC(GFunc.NEDec(drDis["ItmAmtF"], 0) * -1, GFunc.NEDec(row["ItmQty"], 0), GVar.RndDecs.Prcpt);
                            //                    }
                            //                    else
                            //                    {

                            //                        DataRow[] drTot = dtItems.Select("ItmType in(820,825,830) and ItmSN<" + disSN + " and ItmSN>" + totalSN, "ItmSN ASC");

                            //                        if (drTot.Count() > 0)
                            //                        {
                            //                            decimal? sum = GFunc.NEDec(dtItems.Compute("Sum(ItmAmtH)", "(ItmType=100 or ItmType=600) and ItmSN<" + disSN + " and ItmSN>" + totalSN), 0);

                            //                            if (GFunc.NEDec(drTot[0]["ItmSN"], 0) + 1 == disSN)
                            //                            {
                            //                                // decimal ItmAddDisAmtH = GFunc.RndDC(GFunc.NEDec(row["ItmAmtH"], 0), GFunc.NEDec(drTot[0]["ItmAmtShw"], 0) * DocCurrRate, GVar.RndDecs.Curpt);
                            ...
                            //                            else
                            //                            {
                            //                                if (GFunc.NEDec(drTot[0]["ItmSN"], 0) == nextSN)
                            //                                    totalSN = nextSN;
                            ...
                            //        prevSN = disSN;
                            //    }
                            //    DataRow[] drlasts = dtItems.Select("ItmSN>" + prevSN + " and ItmAddDisF<>0", "ItmSN ASC");
                            //    if (drlasts.Length > 0)
                            //    {
                            //        for (int x = 0; x < drlasts.Length; x++)
                            //        {
                            //            drlasts[x]["ItmAddDisAmtH"] = 0;
                            //            drlasts[x]["ItmAddDisAmtF"] = 0;
                            //            drlasts[x]["ItmAddDisH"] = 0;
                            //            drlasts[x]["ItmAddDisF"] = 0;
                            //        }
                            //    }
                            //}
                            //else
                            //{
                            //    dtItemFilter = dtItems.AsEnumerable().Where(r => !GFunc.IsNEZ(r.Field<decimal?>("ItmAddDisAmtH")) || !GFunc.IsNEZ(r.Field<decimal?>("ItmAddDisH")));
                            //    foreach (DataRow row in dtItemFilter)
                            //    {
                            //        row["ItmAddDisAmtH"] = 0;
                            //        row["ItmAddDisAmtF"] = 0;
                    ...
                    switch (objDoc.DocCodeKey)
                    {
                        case (int)GEnum.SystemCode.Purchase_Invoice:
                        case (int)GEnum.SystemCode.Purchase_Debit_Note:
                        case (int)GEnum.SystemCode.Purchase_Order: //added by Jane on 09-May-2025

                            #region calculating additional discount for all detail if there is.. Added by May on 21 Dec 2018
                            if (dtItems.Rows.Count == 0)
                                break;

                            DataTable dtW = dtItems.Clone();
                            IEnumerable<DataRow> dtWFilter = dtItems.AsEnumerable().Where(r => r.Field<int>("LineLinkKey") == 0 && r.Field<int>("ItmType") != 800 && r.Field<int>("ItmType") != 810).OrderBy(r => r.Field<decimal>("ItmSN"));

                            if(dtWFilter.Count()>0)
                                dtW = dtWFilter.CopyToDataTable();

                            DataRow[] drs = dtW.Select("ItmType=710", "ItmSN ASC");
                            ...
                            if (drs.Length > 0)
                            {
                                ...
                                {
                                    decimal disSN = GFunc.NEDec(drDis["ItmSN"], 0);
                                    int disIndex = dtW.Rows.IndexOf(drDis);

                                    dtItemFilter = dtItems.AsEnumerable().Where(r => r.Field<int>("LineLinkKey") == 0 && r.Field<int>("ItmType") != 800 && r.Field<int>("ItmType") != 810 
                                        && r.Field<decimal>("ItmSN") < disSN && r.Field<decimal>("ItmSN") > prevSN).OrderBy(r => r.Field<decimal>("ItmSN"));                                    

                                    int totalRowsCount = dtItemFilter.Count();
                                    ...
                                    {
                                        DataRow row = dtItemFilter.ElementAt(i);
                                        switch (GFunc.GetINTypeGroup(GFunc.NEInt(row["ItmType"], 0)))
                                        {
                                            case (int)GEnum.INTypeGrp.Stock:
                                            case (int)GEnum.INTypeGrp.Non_Stock:
                                                ...
                                                {
                                                    DataRow[] drCFTot = dtW.Select("ItmType=830 and ItmSN<" + disSN + " and ItmSN>" + totalSN, "ItmSN ASC");

                                                    if (drCFTot.Count() > 0)
                                                    {
                                                        decimal? sum = GFunc.NEDec(dtW.Compute("Sum(ItmAmtH)", "(ItmType=100 or ItmType=600) and ItmSN<" + disSN + " and ItmSN>" + totalSN), 0);

                                                        //if (GFunc.NEDec(drCFTot[0]["ItmSN"], 0) + 1 == disSN)
                                                        if(dtW.Rows.IndexOf(drCFTot[0])+1== disIndex)
                                                        {
                                                            // decimal ItmAddDisAmtH = GFunc.RndDC(GFunc.NEDec(row["ItmAmtH"], 0), GFunc.NEDec(drTot[0]["ItmAmtShw"], 0) * DocCurrRate, GVar.RndDecs.Curpt);
                                                            decimal ItmAddDisAmtH = GFunc.RndDC(GFunc.NEDec(row["ItmAmtH"], 0), sum, GVar.RndDecs.Curpt);
                                                        ...
                                                        {
                                                            if (GFunc.NEDec(drCFTot[0]["ItmSN"], 0) == nextSN)
                                                                totalSN = nextSN;
                                                    ...
                                                    {                                                       
                                                        DataRow[] drTot = dtW.Select("ItmType in(820,825) and ItmSN<" + disSN + " and ItmSN>" + totalSN, "ItmSN ASC");
                                                        ...
                                                        if (drTot.Count() > 0)
                                                        {
                                                            ...
                                                            //if (GFunc.NEDec(drTot[0]["ItmSN"], 0) + 1 == disSN)
                                                            if (dtW.Rows.IndexOf(drTot[0]) + 1 == disIndex)
                                                            {
                                                                // decimal ItmAddDisAmtH = GFunc.RndDC(GFunc.NEDec(row["ItmAmtH"], 0), GFunc.NEDec(drTot[0]["ItmAmtShw"], 0) * DocCurrRate, GVar.RndDecs.Curpt);
                                                                decimal ItmAddDisAmtH = GFunc.RndDC(GFunc.NEDec(row["ItmAmtH"], 0), sum, GVar.RndDecs.Curpt);
                                ...
                                DataRow[] drlasts = dtW.Select("ItmSN>" + prevSN + " and ItmAddDisF<>0", "ItmSN ASC");
                                if (drlasts.Length > 0)
                                {
                                    for (int x = 0; x < drlasts.Length; x++)
                                    {
                            ...
                            {
                                dtItemFilter = dtW.AsEnumerable().Where(r => !GFunc.IsNEZ(r.Field<decimal?>("ItmAddDisAmtH")) || !GFunc.IsNEZ(r.Field<decimal?>("ItmAddDisH")));
                                foreach (DataRow row in dtItemFilter)
                                {
                                    row["ItmAddDisAmtH"] = 0;
                                    row["ItmAddDisAmtF"] = 0;
        ...
        private static bool CalDocCT(SqlConnection cn, Document objDoc, DataTable dtItems, bool RunCheck)
        {
            ...
            try
            {
                ...
                if (RunCheck)
                {
                    ...
                    decimal RndAdjAmt = Math.Abs(GFunc.NEDec(TotalApplyAPAmtH - TotalApplyARAmtH, 0));
                    ...
                    else
                    {
                        if (RndAdjAmt > 0)
                        {
                            MsgBox.Show(cn, "Saved successfully with rounding adjustment " + GFunc.RndC(RndAdjAmt, 2), GEnum.MsgBoxIcon.Information, GEnum.MsgBoxButton.OK);
                        }
                        return true;
                    }
        ...
        private static bool CalDocPY(SqlConnection cn, Document objDoc, DataTable dtExp, DataTable dtItems, bool CalTax, bool RunCheck, bool ColTaxAmt = false)
        {
            #region variable
                            ...
                            */
                            ...
                            if (ExpTaxGrpRate > 0)
                            {
                                if (CalTax)
                                {
                                    if (ColTaxAmt)
                                    {
                                        ExpTaxGrpAmtF = GFunc.RndC(GFunc.NEDec(row["ExpTaxGrpAmtF"], 0), GVar.RndDecs.Amtpt);
