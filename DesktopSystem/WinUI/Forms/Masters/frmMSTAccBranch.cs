...
namespace WinUI
{
    public partial class frmMstAccBranch : Form
    {
        #region Local Variables

        private BOLib.MSTAccBranchFactory objBranchFactory = null;
        ...
        //Initialize
        public frmMstAccBranch()
        {
            InitializeComponent();
        }//Completed
        public frmMstAccBranch(string id)
        {
        ...
        public frmMstAccBranch(int key)
        {
        ...
        public frmMstAccBranch(GEnum.SystemCode DocCodeKey, DataTable dtHeader, DataSet dsDetail)
        {
        ...
        //Form Events
        private void frmMSTAccBranch_Load(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;

            try
            {
                this.tagrdBranch.DisplayLayout.Bands[0].SortedColumns.Clear();
                //Call Initialization
                this.objBranchFactory = new BOLib.MSTAccBranchFactory(BOLib.GEnum.InstanceMode.Normal);
                if (objBranchFactory.GUID <= 0)
                {
                    formClose = true;
                    return;
                }
                ...
                this.objBranchFactory.ErrorNotifierHeader_Set += new GVar.UINotifierEvent(this.ErrorNotifier_Set);
                this.objBranchFactory.ErrorNotifierHeader_Clear += new GVar.UINotifierEvent(this.ErrorNotifier_Clear);
                ...
                {
                    if (objBranchFactory.SetReadOnlyData(_dtHeader, _dsDetail) == false)
                    {
                        formClose = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    else if (formOpenMode == GEnum.formInitMode.Add)
                    {
                        if (canEditRecordID && recordID != string.Empty)
                            this.BranchID.SetValueTrigger(recordID, false);
                    }
                }

                //Setup Grid Layout, ShortcutMenu, fill Combo
                GlobalUI.FormGrids_Set(this, (int)objBranchFactory.ConstantCodeKey, out ContextMenuSetting);
                ContextMenuSetting += GlobalUI.ContextMenuSetting_GetNew((int)objBranchFactory.ConstantCodeKey);
                //GlobalUI.cmnuGlobal_Set(this);
                GlobalUI.Combos_Fill(this, (int)objBranchFactory.ConstantCodeKey);

                //Setup List properties
                this.tagrdBranch.DisplayLayout.Override.AllowDelete = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdBranch.DisplayLayout.Override.AllowUpdate = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdBranch.DisplayLayout.Bands[0].Override.ActiveCellAppearance.BackColor = Color.Gold;
        ...
        private void frmMSTAccBranch_Shown(object sender, EventArgs e)
        {
            //Close Form when FormLoad has encounter errors
            if (formClose)
                this.Close();
        }//Completed
        private void frmMSTAccBranch_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.formClose && objBranchFactory == null)
                return;
            ...
            try
            {
                ...
                if ((bool)this.objBranchFactory.Dispose() == false)
            ...
            {
                if (e.Cancel == false)
                    this.objBranchFactory.Dispose();
            }
        }//Completed        
        private void frmMstAccBranch_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void Refresh_Header()
        {
            try
            {
                this.bdsBranchDet.DataSource = objBranchFactory.ObjMSTAccBranch;
                this.bdsBranchDet.AllowNew = true;
                this.bdsBranchDet.ResetBindings(false);
            }
            catch (TAException tex)
            {
        ...
        private void Refresh_GridList()
        {
            try
            {
                ...
                {
                    string listID = GlobalUI.ListSettingID_Get(ContextMenuSetting, "0GF", tagrdBranch.Name);
                    GlobalUI.Grid_Format(tagrdBranch, listID, true, false);
                }
                if (this.tagrdBranch.DisplayLayout.Bands[0].SortedColumns.Count <= 0)
                    this.tagrdBranch.DisplayLayout.Bands[0].SortedColumns.Add(tagrdBranch.DisplayLayout.Bands[0].Columns["BranchID"], false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void FormLayout()
        {
            try
            {
                bool EnableMode = !this.objBranchFactory.IsReadOnly;
                this.tslReadOnly.Text = !EnableMode ? "Read Only" : string.Empty;

                //Special Condition for RecordID
                this.BranchID.Enabled = EnableMode;
                this.BranchID.ReadOnly = !canEditRecordID;

                this.BranchNm.Enabled = EnableMode;
                ...
                else
                {
                    ...
                    if (this.objBranchFactory.IsNew)
                    {
        ...
        private void ListSelectionSync()
        {
            ...
            try
            {
                if (objBranchFactory.ObjMSTAccBranch.BranchKey > 0)
                {
                    ...
                    if (tagrdBranch.Selected != null)
                        if (tagrdBranch.Selected.Cells.Count > 0)
                            if (GFunc.NEInt(tagrdBranch.Selected.Cells[0].Row.Cells["BranchKey"].Value, 0) == objBranchFactory.ObjMSTAccBranch.BranchKey)
                                return;//already selected.

                    //Select the row on the list to reflect the current value in Header.recordkey
                    UltraGridRow ToSelectRow = this.tagrdBranch.Rows.OfType<UltraGridRow>().ToList().Find(
                    row => row.Cells["BranchKey"].Text.Equals(objBranchFactory.ObjMSTAccBranch.BranchKey.ToString(), StringComparison.CurrentCultureIgnoreCase));
                    if (ToSelectRow != null)
                    {
                        ToSelectRow.Cells["BranchID"].Selected = true;
                        ToSelectRow.Cells["BranchID"].Activate();
                    }
                }
                else
                {
                    tagrdBranch.Selected.Cells.Clear();
                    tagrdBranch.ActiveRow = null;
                }
            }
            catch (TAException tex)
            {
        ...
        private bool New_Process()
        {
            ...
            try
            {
                if (form_CanValidate() == false)
                {
                    ...
                    {
                        this.objBranchFactory.IsDirty = false;
                    }
                }

                this.errorProvider1.Clear();

                if (this.objBranchFactory.IsDirty)
                {
                    if (Save_Process() == false)
                        return false;
                }

                if (this.objBranchFactory.New() == false)
                {                   
                    return false;
                }
                else
        ...
        private bool OpenRecord(int key)
        {
            ...
            try
            {
                if (SECPermUtility.Edit(objBranchFactory.PermID, false))
                {
                    if (objBranchFactory.GetEdit(key) == false)
                    {
                        if (SysOptionUtility.GetBool("WarnOpenRecordAsReadOnly"))
                        {
                            ...
                            {
                                objBranchFactory.GetReadOnly(key);
                            }
                        }
                    }
                }
        ...
        private bool Delete_Process()
        {
            ...
            try
            {
                ...
                if (this.objBranchFactory.Delete())
                {
                    this.objBranchFactory.New();
                  
                    //Move the cursor position of active row index to upper row
                    if (tagrdBranch.ActiveRow.Index > 0)
                        PreRowIndex = tagrdBranch.ActiveRow.Index - 1;    
            ...
            finally
            {
                this.Refresh_All(false);
                this.FormLayout();
                if (tagrdBranch.Rows.Count > 0)
                {
                    tagrdBranch.Rows[PreRowIndex].Selected = true;
                    tagrdBranch.Rows[PreRowIndex].Activate();
                }
                this.Cursor = Cursors.Default;
            }
        }//Completed
        private bool Clear_Process()
        {
            ...
            try
            {
                if (GFunc.IsNEZ(this.objBranchFactory.ObjMSTAccBranch.BranchKey))
                {
        ...
        //Grid Common Events
        private void tagrdBranch_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {
            int key = 0;
            try
            {
                ...
                {
                    if (this.tagrdBranch.ActiveRow != null && tagrdBranch.Selected.Cells.Count > 0)//this line is required to check if user have selected a row in the list
                        key = GFunc.NEInt(tagrdBranch.Selected.Cells[0].Row.Cells["BranchKey"].Value, 0);
        ...
        private void tagrdBranch_KeyDown(object sender, KeyEventArgs e)
        {
