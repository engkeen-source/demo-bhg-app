...
namespace WinUI
{
    public partial class frmMSTVehicle : Form
    {
        ...
        public GEnum.SystemCode SysCode = GEnum.SystemCode.Vehicle;
        frmList fMSTVehicleList = null;

        #endregion

        //Initialize
        public frmMSTVehicle()
        {
            InitializeComponent();
        }//Completed
        public frmMSTVehicle(string id)
        {
            InitializeComponent();
            recordID = id;
        ...
        public frmMSTVehicle(GEnum.SystemCode DocCodeKey, DataTable dtHeader, DataSet dsDetail)
        {
        ...
        //Form Events
        private void frmMSTVehicle_Load(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;

            try
            {
                this.tagrdVehicle.DisplayLayout.Bands[0].SortedColumns.Clear();
                //Call Initialization
                this.objVehicleFactory = new BOLib.MSTVehicleFactory(BOLib.GEnum.InstanceMode.Normal);
                if (objVehicleFactory.GUID <= 0)
                {
                    formClose = true;
                    return;
                }
                ...
                this.objVehicleFactory.ErrorNotifierHeader_Set += new GVar.UINotifierEvent(this.ErrorNotifier_Set);
                this.objVehicleFactory.ErrorNotifierHeader_Clear += new GVar.UINotifierEvent(this.ErrorNotifier_Clear);
                ...
                {
                    if (objVehicleFactory.SetReadOnlyData(_dtHeader, _dsDetail) == false)
                    {
                        formClose = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    else if (formOpenMode == GEnum.formInitMode.Add)
                    {
                        if (canEditRecordID && recordID != string.Empty)
                            this.Vehicle.SetValueTrigger(recordID, false);
                    }
                }

                //Setup Grid Layout, ShortcutMenu, fill Combo
                GlobalUI.FormGrids_Set(this, (int)objVehicleFactory.ConstantCodeKey, out ContextMenuSetting);
                ContextMenuSetting += GlobalUI.ContextMenuSetting_GetNew((int)objVehicleFactory.ConstantCodeKey);
                //GlobalUI.cmnuGlobal_Set(this);
                GlobalUI.Combos_Fill(this, (int)objVehicleFactory.ConstantCodeKey);

                //Setup List properties
                this.tagrdVehicle.DisplayLayout.Override.AllowDelete = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdVehicle.DisplayLayout.Override.AllowUpdate = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdVehicle.DisplayLayout.Bands[0].Override.ActiveCellAppearance.BackColor = Color.Gold;
        ...
        private void frmMSTVehicle_Shown(object sender, EventArgs e)
        {
            //Close Form when FormLoad has encounter errors
            if (formClose)
                this.Close();
        }//Completed
        private void frmMSTVehicle_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.formClose && objVehicleFactory == null)
                return;
            try
            {
                ...
                //Dispose Factory
                if ((bool)this.objVehicleFactory.Dispose() == false)
            ...
            {
                if (e.Cancel == false)
                    this.objVehicleFactory.Dispose();
            }
        }//Completed
        private void frmMSTVehicle_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void tsbImport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                frmVehicleDataImport dataImport = new frmVehicleDataImport();
        ...
        private void Refresh_Header()
        {
            try
            {
                this.bdsVehicle.DataSource = objVehicleFactory.ObjMSTVehicle;
                this.bdsVehicle.AllowNew = true;
                this.bdsVehicle.ResetBindings(false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void Refresh_GridList()
        {
            try
            {
                ...
                {
                    string listID = GlobalUI.ListSettingID_Get(ContextMenuSetting, "0GF", tagrdVehicle.Name);
                    GlobalUI.Grid_Format(tagrdVehicle, listID, true, false);
                }
                this.tagrdVehicle.DataSource = BOLib.MASList.GetVehicle(out msgID);
                if (this.tagrdVehicle.DisplayLayout.Bands[0].SortedColumns.Count <= 0)
                    this.tagrdVehicle.DisplayLayout.Bands[0].SortedColumns.Add(tagrdVehicle.DisplayLayout.Bands[0].Columns["Vehicle"], false);
        ...
        {
            bool EnableMode = !this.objVehicleFactory.IsReadOnly;
            this.tslReadOnly.Text = !EnableMode ? "Read Only" : string.Empty;

            //Special Condition for RecordID
            this.Vehicle.Enabled = EnableMode;
            this.Vehicle.ReadOnly = !canEditRecordID;

            this.ChassisNo.Enabled = EnableMode;
            ...
            else
            {
                ...
                if (this.objVehicleFactory.IsNew)
                {
        ...
        private void ListSelectionSync()
        {
            ...
            try
            {
                if (objVehicleFactory.ObjMSTVehicle.VehicleKey > 0)
                {
                    ...
                    if (tagrdVehicle.Selected != null)
                        if (tagrdVehicle.Selected.Cells.Count > 0)
                            if (GFunc.NEInt(tagrdVehicle.Selected.Cells[0].Row.Cells["VehicleKey"].Value, 0) == objVehicleFactory.ObjMSTVehicle.VehicleKey)
                                return;//already selected.

                    //Select the row on the list to reflect the current value in Header.recordkey
                    UltraGridRow ToSelectRow = this.tagrdVehicle.Rows.OfType<UltraGridRow>().ToList().Find(
                    row => row.Cells["VehicleKey"].Text.Equals(objVehicleFactory.ObjMSTVehicle.VehicleKey.ToString(), StringComparison.CurrentCultureIgnoreCase));
                    if (ToSelectRow != null)
                    {
                        ToSelectRow.Cells["Vehicle"].Selected = true;
                        ToSelectRow.Cells["Vehicle"].Activate();
                    }
                }
                else
                {
                    tagrdVehicle.Selected.Cells.Clear();
                    tagrdVehicle.ActiveRow = null;
                }
            }
            catch (TAException tex)
            {
        ...
        private bool New_Process()
        {
            ...
            try
            {
                if (form_CanValidate() == false)
                {
                    ...
                    {
                        this.objVehicleFactory.IsDirty = false;
                    }
                }

                this.errorProvider1.Clear();

                if (this.objVehicleFactory.IsDirty)
                {
                    if (Save_Process() == false)
                        return false;
                }

                if (this.objVehicleFactory.New() == false)
                {
                    return false;
                }
                else
        ...
        private bool Save_Process()
        {
            ...
            try
            {
                ...
                //Perform Saving
                if (this.objVehicleFactory.Save())
                {
                    GlobalUI.ResetControlDirty(this);
                    return true;
                }
        ...
        private bool OpenRecord(int key)
        {
            ...
            try
            {
                if (SECPermUtility.Edit(objVehicleFactory.PermID, false))
                {
                    if (objVehicleFactory.GetEdit(key) == false)
                    {
                        if (SysOptionUtility.GetBool("WarnOpenRecordAsReadOnly"))
                        {
                            ...
                            {
                                objVehicleFactory.GetReadOnly(key);
                            }
                        }
                    }
                }
        ...
        private bool Delete_Process()
        {
            ...
            try
            {
                ...
                if (this.objVehicleFactory.Delete())
                {
                    this.objVehicleFactory.New();

                    //Move the cursor position of active row index to upper row
                    if (tagrdVehicle.ActiveRow.Index > 0)
                        PreRowIndex = tagrdVehicle.ActiveRow.Index - 1;
            ...
            finally
            {
                ...
                if (tagrdVehicle.Rows.Count > 0)
                {
                    tagrdVehicle.Rows[PreRowIndex].Selected = true;
                    tagrdVehicle.Rows[PreRowIndex].Activate();
                }
                this.Cursor = Cursors.Default;
            }
        }//Completed
        private bool Clear_Process()
        {
            ...
            try
            {
                if (GFunc.IsNEZ(this.objVehicleFactory.ObjMSTVehicle.VehicleKey))
                {
        ...
        //        if (SECPermUtility.Edit(objVehicleFactory.PermID, false))
        //        {
        //            if (objVehicleFactory.GetEdit(key, id) == false)
        //            {
        //                if (SysOptionUtility.GetBool("WarnOpenRecordAsReadOnly"))
        //                {
        //                    btnSelect = MsgBox.Show("Try to open record as read only",
        ...
        //                    if (btnSelect == GEnum.MsgBoxButton.Yes)
        //                    {
        //                        objVehicleFactory.GetReadOnly(key, id);
        //                    }
        //                }
        //            }
        //        }
        ...
        private void tagrdVehicle_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void tagrdVehicle_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {
            int key = 0;
            try
            {
                ...
                {
                    if (this.tagrdVehicle.ActiveRow != null && tagrdVehicle.Selected.Cells.Count > 0)//this line is required to check if user have selected a row in the list
                        key = GFunc.NEInt(tagrdVehicle.Selected.Cells[0].Row.Cells["VehicleKey"].Value, 0);
        ...
        //Custom Event
        private void MYear_CustomUpdate(object sender, CancelEventArgs e)
        {
            if (MYear.Value == null)
            {
                MYear.SetValueTrigger(0, false);
            }
        }
        ...
        private void OpenID_CustomUpdate(object sender, CancelEventArgs e)
        {
            ...
            try
            {
                if (GFunc.IsNE(OpenID.Text) == false)
                {
                    ...
                    RecAccessType = GEnum.RecAccessType.Vehicle;
                    PopupType = (int)GEnum.PopupType.Vehicle;

                    key = GFunc.ConRecord_GetKey(RecAccessType, listSettingID, OpenID.Text, ref id, ref des, true);
                    if (GFunc.IsNEZ(key))
                    {
                        if (DocHDRUtil.EditorButton_Popup((int)objVehicleFactory.ConstantCodeKey, OpenID.Text, listSettingID, PopupType, ref key, ref id, ref des) == false)
                            return;
                    }
        ...
        private void OpenID_EditorButtonClick(object sender, EditorButtonEventArgs e)
        {
            ...
            {
                string listSettingID = GlobalUI.ListSettingID_Get(ContextMenuSetting, OpenID.Name);
                if (objVehicleFactory.ConstantCodeKey == GEnum.SystemCode.Vehicle)
                {
                    if (DocHDRUtil.EditorButton_Popup((int)objVehicleFactory.ConstantCodeKey, OpenID.Text, listSettingID, (int)GEnum.PopupType.Vehicle, ref key, ref id, ref des))
                    {
                        OpenID.SetValueTrigger(des, false);
        ...
        private void OnListRecordSelected(int key)
        {
            ...
            try
            {
                if (this.OpenRecord(key))
                    this.Focus();
                else
                    fMSTVehicleList.Focus();
            }
            catch (TAException tex)
            {
                Error(tex, true);
