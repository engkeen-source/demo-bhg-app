...
namespace WinUI
{
    public partial class frmInsertCPO : Form
    {       
        ...
        public frmInsertCPO(Document doc, TAUtil.TAGridEditor tagrdDetItms)
        {
            InitializeComponent();
        ...
        //Form Events
        private void frmInsertCPO_Load(object sender, EventArgs e)
        {
            try
            {               
        ...
        private void frmInsertCPO_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void frmInsertCPO_FormClosed(object sender, FormClosedEventArgs e)
        {
        ...
        //Grid Events        
        private void tagrdItemCPO_CustomCellUpdate(object sender, BeforeCellUpdateEventArgs e)
        {
            UltraGridCell curCell = tagrdItemCPO.ActiveCell;
                ...
                switch (curCell.Column.Key.ToLower())
                {
                    case "itmqtyreceive":
                        if (GFunc.IsNEZ(curCell.Value))
                            tagrdItemCPO.ActiveRow.Cells["Selected"].Value = 0;
                        else
                        {
                            ...
                            tagrdItemCPO.ActiveRow.Cells["Selected"].Value = 1;
                            if (PickSeq.Checked)
                            {
                                vpickSeq++;
                                tagrdItemCPO.ActiveRow.Cells["pickSeq"].Value = vpickSeq;
                            }
                        }
                        break;
                    case "selected":
                        if (tagrdItemCPO.ActiveRow != null)
                        {
                            Selected_Set(tagrdItemCPO.ActiveRow, (bool)curCell.Value);
                            tagrdItemCPO.UpdateData();
                        }
                        //if (PickSeq.Checked)
                        //{
                        //    vpickSeq += 1;
        ...
        private void btnAppend_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (this.tagrdItemCPO.DataSource != null)
                {
                    DataTable dt = tagrdItemCPO.DataSource as DataTable;
                    ...
                    ////Filter to get data for locking
                    ...
                    paraList.Add(new SqlParameter("@SourceDocCodeKey", (int)GEnum.SystemCode.Order_Consignment ));
                    ...
                    DataTable dtAppend = dtFiltersToLock.Select(r => new { DocKey = r.Field<int>("DocKey"), DocItmKey = r.Field<int>("DocItmKey"), QtySelect = r.Field<decimal>("ItmQtyReceive"), PickSeq = r.Field<int>("PickSeq") }).AsDataTable();
                    if (this.AppendData(dtAppend))
                    {
                        ...
                        DataTable dtCPO = tagrdItemCPO.DataSource as DataTable;
                        for (int i = dtCPO.Rows.Count - 1; i > -1; i--)
                        {
                            if ((bool)dtCPO.Rows[i]["Selected"])
                            {
                                dtCPO.Rows[i].Delete();
                            }
                        }
            ...
            {
                SysLockUtility.RemoveLock(true, GEnum.SysLockOption.ByCodeKeyAndGUID, GEnum.SystemCode.Order_Consignment, objDoc.GUID, 0, 0);
                Error(tex, true);                
            }
            catch (Exception ex)
            {
        ...
        private void btnSelectAll_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (UltraGridRow row in tagrdItemCPO.Rows.GetFilteredInNonGroupByRows())
                {
        ...
        private void btnUnSelectAll_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                foreach (UltraGridRow row in tagrdItemCPO.Rows)
                {
        ...
        private void ComboItm_Fill()
        {
            ...
            if (tagrdItemCPO.DataSource != null)
            {
                DataTable dt = null;
                dt = ((DataTable)tagrdItemCPO.DataSource).DefaultView.ToTable();
        ...
        private bool AppendData(DataTable dtSelected)
        {
            try
            {
                int soureceDocCodeKey = (int)GEnum.SystemCode.Order_Consignment;
                ...
                paraList.Add(new SqlParameter("@InsertAction", GEnum.InsertAction.InsertCO));             
                ...
                if (PickSeq.Checked) 
                    ...
                    dtInsertData, objDoc, tagrdDetItms, (int)GEnum.InsertAction.InsertCO, "PickSeq,ItmSN", UseSystemPrice.Checked, false);

                else 
                    DocHDRUtil.DocTransferData(soureceDocCodeKey, SourceDocKey, SourceDocConKey,
                    dtInsertData, objDoc, tagrdDetItms, (int)GEnum.InsertAction.InsertCO, "ItmSN", UseSystemPrice.Checked, false);
        ...
        {
            GlobalUI.GridAllColumnsActivateOnlySet(tagrdItemCPO);

            tagrdItemCPO.DisplayLayout.Bands[0].Columns["Selected"].Header.VisiblePosition = 0;
            tagrdItemCPO.DisplayLayout.Bands[0].Columns["Selected"].CellActivation = Activation.AllowEdit;            
            tagrdItemCPO.DisplayLayout.Bands[0].Columns["ItmQtyReceive"].CellActivation = Activation.AllowEdit;
        ...
        {
            string listSetingID = "frmInsertCPOGrid";
            try
            {
                GlobalUI.Grid_Format(tagrdItemCPO, listSetingID, true, false);
                vpickSeq = 0;
                RemoveMatchedRow();
                //reload combo data
                ComboDocID_Fill();
        ...
        private void Grid_Filter()
        {
            try
            {
                ...
                //Filter Grid
                //tagrdItemCPO.DisplayLayout.Bands[0].ColumnFilters.ClearAllFilters();
                //ColumnFiltersCollection colFilterCollections = tagrdItemCPO.DisplayLayout.Bands[0].ColumnFilters;
                
                //GridFilterToDefaultView   
                if (docKey > 0)                  
                    ((DataTable)tagrdItemCPO.DataSource).DefaultView.RowFilter = "DocKey=" + docKey;

                if (itmKey > 0)                    
                    ((DataTable)tagrdItemCPO.DataSource).DefaultView.RowFilter = "ItmKey=" + itmKey;

                ((DataTable)tagrdItemCPO.DataSource).DefaultView.Sort = "ItmSN";
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void RemoveMatchedRow()
        {
            ...
            DataTable dtCO = tagrdItemCPO.DataSource as DataTable;
            DataTable dtTgtDetail = tagrdDetItms.DataSource as DataTable;

            var local = from row in dtCO.AsEnumerable()
                        select new
                        {
                            DocKey = row.Field<int>("DocKey"),
            ...
            var caller = from row in dtTgtDetail.AsEnumerable()
                         select new
                         {
                             DocKey = row.Field<int>("CSCPODK"),
                             DocItmKey = row.Field<int>("CSCPODItm")
                         };
            ...
            dtCO.DefaultView.Sort = "DocKey,DocItmKey";
            #endregion

            //remove those rows matched with caller grid
            for (int i = dtCO.Rows.Count - 1; i > -1; i--)
            {
                var query = dtexisting.AsEnumerable().Where(p => p.Field<int>("DocKey") == GFunc.NEInt(dtCO.Rows[i]["DocKey"], 0) && p.Field<int>("DocItmKey") == GFunc.NEInt(dtCO.Rows[i]["DocItmKey"], 0));
                if (query.Count() > 0)
                {
                    dtCO.Rows[i].Delete();
                }
