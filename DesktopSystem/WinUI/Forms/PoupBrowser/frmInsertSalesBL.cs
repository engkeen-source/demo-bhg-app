...
namespace WinUI
{
    public partial class frmInsertSalesBL : Form  
    {
        ...
        //Initialize
        public frmInsertSalesBL()
        {
            InitializeComponent();
        }//Completed
        public frmInsertSalesBL(Document doc, UltraGrid tagrdDetItms,int PODocKey,int PODocItmKey)
        {
            InitializeComponent();
        ...
        public frmInsertSalesBL(UltraGrid tagrdDetItms, int PODocKey, int PODocItmKey)
        {
            InitializeComponent();
        ...
        //Form Events
        private void frmInsertSalesBL_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        private void btnLink_Click(object sender, EventArgs e)
        {
            try
            {
                this.tagrdItemBL.PerformAction(UltraGridAction.ExitEditMode);
                this.tagrdItemBL.UpdateData();

                if(this.tagrdItemBL.Selected.Rows.Count>0)
                {
                    if (GFunc.IsPostingItmType(tagrdItemBL.Selected.Rows[0].Cells["ItmType"].Value))
                    {
                        tagrdDetItms.ActiveRow.Cells["NSLink"].Value = "13500-" + PODocKey + "-" +PODocItmKey;
                        if (!CallfromNSLink)
                        {
                            tagrdDetItms.ActiveRow.Cells["APPOID"].Value = tagrdItemBL.Selected.Rows[0].Cells["DocID"].Value;
                            tagrdDetItms.ActiveRow.Cells["APPOID"].Tag = tagrdItemBL.Selected.Rows[0].Cells["DocID"].Value;
                        }
                        else
                        {
                            tagrdDetItms.ActiveRow.Cells["ItmPOID"].Value = tagrdItemBL.Selected.Rows[0].Cells["DocID"].Value;
                            tagrdDetItms.ActiveRow.Cells["ItmPOSN"].Value = tagrdItemBL.Selected.Rows[0].Cells["ItmSN"].Value;
                            tagrdDetItms.ActiveRow.Cells["ItmPODK"].Value = tagrdItemBL.Selected.Rows[0].Cells["DocKey"].Value;
                            tagrdDetItms.ActiveRow.Cells["ItmPODItm"].Value = tagrdItemBL.Selected.Rows[0].Cells["DocItmKey"].Value;

                            if (GFunc.IsNEZ(tagrdItemBL.Selected.Rows[0].Cells["ItmQty"].Value))
                                tagrdDetItms.ActiveRow.Cells["ItmVendorPriceH"].Value = tagrdItemBL.Selected.Rows[0].Cells["ItmAmtH"].Value;
                            else
                                tagrdDetItms.ActiveRow.Cells["ItmVendorPriceH"].Value =GFunc.RndDC(tagrdItemBL.Selected.Rows[0].Cells["ItmAmtH"].Value,
                                    tagrdItemBL.Selected.Rows[0].Cells["ItmQty"].Value, 6);
        ...
        private void FormLayout()
        {
            ...
            GlobalUI.GridAllColumnsActivateOnlySet(tagrdItemBL);
                    
            tagrdItemBL.DisplayLayout.Bands[0].Columns["ItmQtyReceive"].CellActivation = Activation.AllowEdit;
            tagrdItemBL.DisplayLayout.Bands[0].Override.CellClickAction = Infragistics.Win.UltraWinGrid.CellClickAction.CellSelect;
        ...
        private void ComboDocID_Fill()
        {
            try
            {
                ...
                {
                    if (tagrdItemBL.DataSource != null)
                    {
                        DataTable dt = null;
                        dt = ((DataTable)tagrdItemBL.DataSource).DefaultView.ToTable();
        ...
        private void Grid_Refresh()
        {
            ...
            string listSetingID = "frmInsertSalesBLGrid";
            try
            {
                GlobalUI.Grid_Format(tagrdItemBL, listSetingID, true, false);                                       
                ComboDocID_Fill();               
            }
            catch (TAException tex)
            {
        ...
        private void Grid_Filter()
        {
            try
            {
                ...
                if (docKey > 0)
                {
                    ((DataTable)tagrdItemBL.DataSource).DefaultView.RowFilter = "DocKey=" + docKey;
                    if (ConKey == 0)
                    {
                        if (((DataTable)tagrdItemBL.DataSource).DefaultView.Count > 0)
                        {
                            ConKey1.Value = ((DataTable)tagrdItemBL.DataSource).DefaultView[0]["DocConKey"];
                            ConKey = GFunc.NEInt(ConKey1.Value, 0);
                            foreach(DataRowView dr in ((DataTable)tagrdItemBL.DataSource).DefaultView)
                ...
                ((DataTable)tagrdItemBL.DataSource).DefaultView.Sort = "ItmSN";
        ...
        private void tagrdItemBL_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {          
            if (tagrdItemBL.Selected != null)
            {
                Infragistics.Win.UltraWinGrid.UltraGridRow r = null;
                if (tagrdItemBL.Selected.Rows.Count > 0)
                    r = tagrdItemBL.Selected.Rows[0];
                else if(tagrdItemBL.Selected.Cells.Count>0)
                    r = tagrdItemBL.Selected.Cells[0].Row;
                if (r != null)
                {
        ...
        private void tagrdItemBL_DoubleClickRow(object sender, DoubleClickRowEventArgs e)
        {
