...
namespace WinUI
{
    public partial class frmREFIDFormat : Form
    {
        #region Local Variables
        private BOLib.REFIDFormatFactory objIDFormatFactory = null;
        ...
        //Initialize
        public frmREFIDFormat()
        {
            InitializeComponent();
        }//Completed
        public frmREFIDFormat(string id)
        {
            InitializeComponent();
            recordID = id;
        ...
        public frmREFIDFormat(GEnum.SystemCode DocCodeKey, DataTable dtHeader, DataSet dsDetail)
        {
        ...
        //Form Events
        private void frmREFIDFormat_Load(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;

            try
            {
                this.tagrdIDFormat.DisplayLayout.Bands[0].SortedColumns.Clear();
                //Call Initialization
                this.objIDFormatFactory = new BOLib.REFIDFormatFactory(BOLib.GEnum.InstanceMode.Normal);
                if (objIDFormatFactory.GUID <= 0)
                {
                    formClose = true;
                    return;
                }
                ...
                this.objIDFormatFactory.ErrorNotifierHeader_Set += new GVar.UINotifierEvent(this.ErrorNotifier_Set);
                this.objIDFormatFactory.ErrorNotifierHeader_Clear += new GVar.UINotifierEvent(this.ErrorNotifier_Clear);
                ...
                {
                    if (objIDFormatFactory.SetReadOnlyData(_dtHeader, _dsDetail) == false)
                    {
                        formClose = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    else if (formOpenMode == GEnum.formInitMode.Add)
                    {
                        if (canEditRecordID && recordID != string.Empty)
                            this.IDFormatID.SetValueTrigger(recordID, false);
                    }
                }

                //Setup Grid Layout, ShortcutMenu, fill Combo
                GlobalUI.FormGrids_Set(this, (int)objIDFormatFactory.ConstantCodeKey, out ContextMenuSetting);
                ContextMenuSetting += GlobalUI.ContextMenuSetting_GetNew((int)objIDFormatFactory.ConstantCodeKey);
                //GlobalUI.cmnuGlobal_Set(this);
                GlobalUI.Combos_Fill(this, (int)objIDFormatFactory.ConstantCodeKey);

                //Setup List properties
                this.tagrdIDFormat.DisplayLayout.Override.AllowDelete = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdIDFormat.DisplayLayout.Override.AllowUpdate = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdIDFormat.DisplayLayout.Bands[0].Override.ActiveCellAppearance.BackColor = Color.Gold;
        ...
        private void frmREFIDFormat_Shown(object sender, EventArgs e)
        {
            //Close Form when FormLoad has encounter errors
            if (formClose)
                this.Close();
        }//Completed
        private void frmREFIDFormat_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.formClose && objIDFormatFactory == null)
                return;
            ...
            try
            {
                ...
                //Dispose Factory
                if ((bool)this.objIDFormatFactory.Dispose() == false)
            ...
            {
                if (e.Cancel == false)
                    this.objIDFormatFactory.Dispose();
            }
        }//Completed
        private void frmREFIDFormat_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void Refresh_Header()
        {
            try
            {
                this.bdsIDFormatDet.DataSource = objIDFormatFactory.ObjREFIDFormat;
                this.bdsIDFormatDet.AllowNew = true;
                this.bdsIDFormatDet.ResetBindings(false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void Refresh_GridList()
        {
            try
            {
                ...
                {
                    string listID = GlobalUI.ListSettingID_Get(ContextMenuSetting, "0GF", tagrdIDFormat.Name);
                    GlobalUI.Grid_Format(tagrdIDFormat, listID, true, false);
                }
                if (this.tagrdIDFormat.DisplayLayout.Bands[0].SortedColumns.Count <= 0)
                    this.tagrdIDFormat.DisplayLayout.Bands[0].SortedColumns.Add(tagrdIDFormat.DisplayLayout.Bands[0].Columns["IDFormatID"], false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        {
            bool EnableMode = !this.objIDFormatFactory.IsReadOnly;
            this.tslReadOnly.Text = !EnableMode ? "Read Only" : string.Empty;

            //Special Condition for RecordID
            this.IDFormatID.Enabled = EnableMode;
            this.IDFormatID.ReadOnly = !canEditRecordID;

            this.IDFormatText.Enabled = EnableMode;
            this.IDFormatDes.Enabled = EnableMode;
            ...
            else
            {
                ...
                if (this.objIDFormatFactory.IsNew)
                {
        ...
        private void ListSelectionSync()
        {
            ...
            try
            {
                if (objIDFormatFactory.ObjREFIDFormat.IDFormatKey > 0)
                {
                    ...
                    if (tagrdIDFormat.Selected != null)
                        if (tagrdIDFormat.Selected.Cells.Count > 0)
                            if (GFunc.NEInt(tagrdIDFormat.Selected.Cells[0].Row.Cells["IDFormatKey"].Value, 0) == objIDFormatFactory.ObjREFIDFormat.IDFormatKey)
                                return;//already selected.

                    //Select the row on the list to reflect the current value in Header.recordkey
                    UltraGridRow ToSelectRow = this.tagrdIDFormat.Rows.OfType<UltraGridRow>().ToList().Find(
                    row => row.Cells["IDFormatKey"].Text.Equals(objIDFormatFactory.ObjREFIDFormat.IDFormatKey.ToString(), StringComparison.CurrentCultureIgnoreCase));
                    if (ToSelectRow != null)
                    {
                        ToSelectRow.Cells["IDFormatID"].Selected = true;
                        ToSelectRow.Cells["IDFormatID"].Activate();
                    }
                }
                else
                {
                    tagrdIDFormat.Selected.Cells.Clear();
                    tagrdIDFormat.ActiveRow = null;
                }
            }
            catch (TAException tex)
            {
        ...
        private bool New_Process()
        {
            ...
            try
            {
                if (form_CanValidate() == false)
                {
                    ...
                    {
                        this.objIDFormatFactory.IsDirty = false;
                    }
                }

                this.errorProvider1.Clear();

                if (this.objIDFormatFactory.IsDirty)
                {
                    if (Save_Process() == false)
                        return false;
                }

                if (this.objIDFormatFactory.New() == false)
                {                   
                    return false;
                }
                else
        ...
        private bool Save_Process()
        {
            ...
            try
            {
                ...
                //Perform Saving
                if (this.objIDFormatFactory.Save())
                {
                    GlobalUI.ResetControlDirty(this);
                    return true;
                }
        ...
        private bool OpenRecord(int key)
        {
            ...
            try
            {
                if (SECPermUtility.Edit(objIDFormatFactory.PermID, false))
                {
                    if (objIDFormatFactory.GetEdit(key) == false)
                    {
                        if (SysOptionUtility.GetBool("WarnOpenRecordAsReadOnly"))
                        {
                            ...
                            {
                                objIDFormatFactory.GetReadOnly(key);
                            }
                        }
                    }
                }
        ...
        private bool Delete_Process()
        {
            ...
            try
            {
                ...
                if (this.objIDFormatFactory.Delete())
                {
                    this.objIDFormatFactory.New();
                   
                    //Move the cursor position of active row index to upper row
                    if (tagrdIDFormat.ActiveRow.Index > 0)
                        PreRowIndex = tagrdIDFormat.ActiveRow.Index - 1;
            ...
            finally
            {
                ...
                if (tagrdIDFormat.Rows.Count > 0)
                {
                    tagrdIDFormat.Rows[PreRowIndex].Selected = true;
                    tagrdIDFormat.Rows[PreRowIndex].Activate();
                }
                this.Cursor = Cursors.Default;
            }
        }//Completed
        private bool Clear_Process()
        {
            ...
            try
            {
                if (GFunc.IsNEZ(this.objIDFormatFactory.ObjREFIDFormat.IDFormatKey))
                {
                    ...
                    if (this.objIDFormatFactory.New())
                    {                       
        ...
        //Grid Events
        private void tagrdIDFormat_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void tagrdIDFormat_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {
            int key = 0;
            try
            {
                ...
                {
                    if (this.tagrdIDFormat.ActiveRow != null && tagrdIDFormat.Selected.Cells.Count > 0)//this line is required to check if user have selected a row in the list
                        key = GFunc.NEInt(tagrdIDFormat.Selected.Cells[0].Row.Cells["IDFormatKey"].Value, 0);
