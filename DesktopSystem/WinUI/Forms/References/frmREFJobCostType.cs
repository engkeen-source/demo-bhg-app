...
namespace WinUI
{
    public partial class frmREFJobCostType : Form
    {
        #region Local Variables
        private BOLib.REFJobCostTypeFactory objJobCostTypeFactory = null;
        ...
        //Initialize
        public frmREFJobCostType()
        {
            InitializeComponent();
        }//Completed
        public frmREFJobCostType(string id)
        {
            InitializeComponent();
            recordID = id;
        ...
        public frmREFJobCostType(int Key)
        {
        ...
        public frmREFJobCostType(GEnum.SystemCode DocCodeKey, DataTable dtHeader, DataSet dsDetail)
        {
        ...
        //Form Events
        private void frmREFJobCostType_Load(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;

            try
            {
                this.tagrdJobCostType.DisplayLayout.Bands[0].SortedColumns.Clear();
                //Call Initialization
                this.objJobCostTypeFactory = new BOLib.REFJobCostTypeFactory(BOLib.GEnum.InstanceMode.Normal);
                if (objJobCostTypeFactory.GUID <= 0)
                {
                    formClose = true;
                    return;
                }
                ...
                this.objJobCostTypeFactory.ErrorNotifierHeader_Set += new GVar.UINotifierEvent(this.ErrorNotifier_Set);
                this.objJobCostTypeFactory.ErrorNotifierHeader_Clear += new GVar.UINotifierEvent(this.ErrorNotifier_Clear);
                ...
                {
                    if (objJobCostTypeFactory.SetReadOnlyData(_dtHeader, _dsDetail) == false)
                    {
                        formClose = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    else if (formOpenMode == GEnum.formInitMode.Add)
                    {
                        if (canEditRecordID && recordID != string.Empty)
                            this.JobCostTypeID.SetValueTrigger(recordID, false);
                    }
                }

                //Setup Grid Layout, ShortcutMenu, fill Combo
                GlobalUI.FormGrids_Set(this, (int)objJobCostTypeFactory.ConstantCodeKey, out ContextMenuSetting);
                ContextMenuSetting += GlobalUI.ContextMenuSetting_GetNew((int)objJobCostTypeFactory.ConstantCodeKey);
                //GlobalUI.cmnuGlobal_Set(this);
                GlobalUI.Combos_Fill(this, (int)objJobCostTypeFactory.ConstantCodeKey);

                //Setup List properties
                this.tagrdJobCostType.DisplayLayout.Override.AllowDelete = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdJobCostType.DisplayLayout.Override.AllowUpdate = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdJobCostType.DisplayLayout.Bands[0].Override.ActiveCellAppearance.BackColor = Color.Gold;
        ...
        private void frmREFJobCostType_Shown(object sender, EventArgs e)
        {
            //Close Form when FormLoad has encounter errors
            if (formClose)
                this.Close();
        }//Completed
        private void frmREFJobCostType_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.formClose && objJobCostTypeFactory == null)
                return;
            try
            {
                ...
                //Dispose Factory
                if ((bool)this.objJobCostTypeFactory.Dispose() == false)
            ...
            {
                if (e.Cancel == false)
                    this.objJobCostTypeFactory.Dispose();
            }
        }//Completed
        private void frmREFJobCostType_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void Refresh_Header()
        {
            try
            {
                this.bdsJobCostTypeDet.DataSource = objJobCostTypeFactory.ObjREFJobCostType;
                this.bdsJobCostTypeDet.AllowNew = true;
                this.bdsJobCostTypeDet.ResetBindings(false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void Refresh_GridList()
        {
            try
            {
                ...
                {
                    string listID = GlobalUI.ListSettingID_Get(ContextMenuSetting, "0GF", tagrdJobCostType.Name);
                    GlobalUI.Grid_Format(tagrdJobCostType, listID, true, false);
                }
                if (this.tagrdJobCostType.DisplayLayout.Bands[0].SortedColumns.Count <= 0)
                    this.tagrdJobCostType.DisplayLayout.Bands[0].SortedColumns.Add(tagrdJobCostType.DisplayLayout.Bands[0].Columns["JobCostTypeID"], false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        {
            bool EnableMode = !this.objJobCostTypeFactory.IsReadOnly;
            this.tslReadOnly.Text = !EnableMode ? "Read Only" : string.Empty;

            //Special Condition for RecordID
            this.JobCostTypeID.Enabled = EnableMode;
            this.JobCostTypeID.ReadOnly = !canEditRecordID;

            this.JobCostHeaderType.Enabled = EnableMode;
            this.JobCostTypeDes.Enabled = EnableMode;
            ...
            else
            {
                ...
                if (this.objJobCostTypeFactory.IsNew)
                {
        ...
        private void ListSelectionSync()
        {
            ...
            try
            {
                if (objJobCostTypeFactory.ObjREFJobCostType.JobCostTypeKey > 0)
                {
                    ...
                    if (tagrdJobCostType.Selected != null)
                        if (tagrdJobCostType.Selected.Cells.Count > 0)
                            if (GFunc.NEInt(tagrdJobCostType.Selected.Cells[0].Row.Cells["JobCostTypeKey"].Value, 0) == objJobCostTypeFactory.ObjREFJobCostType.JobCostTypeKey)
                                return;//already selected.

                    //Select the row on the list to reflect the current value in Header.recordkey
                    UltraGridRow ToSelectRow = this.tagrdJobCostType.Rows.OfType<UltraGridRow>().ToList().Find(
                    row => row.Cells["JobCostTypeKey"].Text.Equals(objJobCostTypeFactory.ObjREFJobCostType.JobCostTypeKey.ToString(), StringComparison.CurrentCultureIgnoreCase));
                    if (ToSelectRow != null)
                    {
                        ToSelectRow.Cells["JobCostTypeID"].Selected = true;
                        ToSelectRow.Cells["JobCostTypeID"].Activate();
                    }
                }
                else
                {
                    tagrdJobCostType.Selected.Cells.Clear();
                    tagrdJobCostType.ActiveRow = null;
                }
            }
            catch (TAException tex)
            {
        ...
        private bool New_Process()
        {
            ...
            try
            {
                if (form_CanValidate() == false)
                {
                    ...
                    {
                        this.objJobCostTypeFactory.IsDirty = false;
                    }
                }

                this.errorProvider1.Clear();

                if (this.objJobCostTypeFactory.IsDirty)
                {
                    if (Save_Process() == false)
                        return false;
                }

                if (this.objJobCostTypeFactory.New() == false)
                {
                    return false;
                }
                else
        ...
        private bool SaveChanges()
        {
            try
            {
                ...
                if (objJobCostTypeFactory.IsDirty)
                {
                    GEnum.MsgBoxButton btnSelect;
        ...
        private bool Save_Process()
        {
            ...
            try
            {
                ...
                //Perform Saving
                if (this.objJobCostTypeFactory.Save())
                {
                    GlobalUI.ResetControlDirty(this);
                    return true;
        ...
        private bool OpenRecord(int key)
        {
            ...
            try
            {
                if (SECPermUtility.Edit(objJobCostTypeFactory.PermID, false))
                {
                    if (objJobCostTypeFactory.GetEdit(key) == false)
                    {
                        if (SysOptionUtility.GetBool("WarnOpenRecordAsReadOnly"))
                        {
                            ...
                            {
                                objJobCostTypeFactory.GetReadOnly(key);
                            }
                        }
                    }
                }
        ...
        private bool Delete_Process()
        {
            ...
            try
            {
                ...
                if (this.objJobCostTypeFactory.Delete())
                {
                    this.objJobCostTypeFactory.New();
                   
                    //Move the cursor position of active row index to upper row
                    if (tagrdJobCostType.ActiveRow.Index > 0)
                        PreRowIndex = tagrdJobCostType.ActiveRow.Index - 1;
            ...
            finally
            {
                Refresh_Form(false);
                FormLayout();
                if (tagrdJobCostType.Rows.Count > 0)
                {
                    tagrdJobCostType.Rows[PreRowIndex].Selected = true;
                    tagrdJobCostType.Rows[PreRowIndex].Activate();
                }
                this.Cursor = Cursors.Default;
            }
        }//Completed
        private bool Clear_Process()
        {
            ...
            try
            {
                if (GFunc.IsNEZ(this.objJobCostTypeFactory.ObjREFJobCostType.JobCostTypeKey))
                {
                    ...
                    if (this.objJobCostTypeFactory.New())
                    {                       
        ...
        //Grid Events
        private void tagrdJobCostType_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void tagrdJobCostType_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {
            int key = 0;
            try
            {
                ...
                {
                    if (this.tagrdJobCostType.ActiveRow != null && tagrdJobCostType.Selected.Cells.Count > 0)//this line is required to check if user have selected a row in the list
                        key = GFunc.NEInt(tagrdJobCostType.Selected.Cells[0].Row.Cells["JobCostTypeKey"].Value, 0);
