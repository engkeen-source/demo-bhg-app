...
namespace WinUI
{
    public partial class frmREFJobGrp : Form
    {
        #region Local Variables
        private BOLib.REFJobGrpFactory objJobGrpFactory = null;
        ...
        public frmREFJobGrp(GEnum.SystemCode DocCodeKey, DataTable dtHeader, DataSet dsDetail)
        {
        ...
        //Form Events
        private void frmREFJobGrp_Load(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;

            try
            {
                this.tagrdJobGrp.DisplayLayout.Bands[0].SortedColumns.Clear();
                //Call Initialization
                this.objJobGrpFactory = new BOLib.REFJobGrpFactory(BOLib.GEnum.InstanceMode.Normal);
                if (objJobGrpFactory.GUID <= 0)
                {
                    formClose = true;
                    return;
                }
                ...
                this.objJobGrpFactory.ErrorNotifierHeader_Set += new GVar.UINotifierEvent(this.ErrorNotifier_Set);
                this.objJobGrpFactory.ErrorNotifierHeader_Clear += new GVar.UINotifierEvent(this.ErrorNotifier_Clear);
                ...
                {
                    if (objJobGrpFactory.SetReadOnlyData(_dtHeader, _dsDetail) == false)
                    {
                        formClose = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    else if (formOpenMode == GEnum.formInitMode.Add)
                    {
                        if (canEditRecordID && recordID != string.Empty)
                            this.JobGrpID.SetValueTrigger(recordID, false);
                    }
                }

                //Setup Grid Layout, ShortcutMenu, fill Combo
                GlobalUI.FormGrids_Set(this, (int)objJobGrpFactory.ConstantCodeKey, out ContextMenuSetting);
                ContextMenuSetting += GlobalUI.ContextMenuSetting_GetNew((int)objJobGrpFactory.ConstantCodeKey);
                //GlobalUI.cmnuGlobal_Set(this);
                GlobalUI.Combos_Fill(this, (int)objJobGrpFactory.ConstantCodeKey);

                //Setup List properties
                this.tagrdJobGrp.DisplayLayout.Override.AllowDelete = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdJobGrp.DisplayLayout.Override.AllowUpdate = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdJobGrp.DisplayLayout.Bands[0].Override.ActiveCellAppearance.BackColor = Color.Gold;
        ...
        private void frmREFJobGrp_Shown(object sender, EventArgs e)
        {
            //Close Form when FormLoad has encounter errors
            if (formClose)
                this.Close();
        }//Completed
        private void frmREFJobGrp_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.formClose && objJobGrpFactory == null)
                return;
            try
            {
                ...
                //Dispose Factory
                if ((bool)this.objJobGrpFactory.Dispose() == false)
            ...
            {
                if (e.Cancel == false)
                    this.objJobGrpFactory.Dispose();
            }
        }//Completed
        private void frmREFJobGrp_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void Refresh_Header()
        {
            try
            {
                this.bdsJobGrpDet.DataSource = objJobGrpFactory.ObjREFJobGrp;
                this.bdsJobGrpDet.AllowNew = true;
                this.bdsJobGrpDet.ResetBindings(false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void Refresh_GridList()
        {
            try
            {
                ...
                {
                    string listID = GlobalUI.ListSettingID_Get(ContextMenuSetting, "0GF", tagrdJobGrp.Name);
                    GlobalUI.Grid_Format(tagrdJobGrp, listID, true, false);
                }
                if (this.tagrdJobGrp.DisplayLayout.Bands[0].SortedColumns.Count <= 0)
                    this.tagrdJobGrp.DisplayLayout.Bands[0].SortedColumns.Add(tagrdJobGrp.DisplayLayout.Bands[0].Columns["JobGrpID"], false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        {
            bool EnableMode = !this.objJobGrpFactory.IsReadOnly;
            this.tslReadOnly.Text = !EnableMode ? "Read Only" : string.Empty;

            //Special Condition for RecordID
            this.JobGrpID.Enabled = EnableMode;
            this.JobGrpID.ReadOnly = !canEditRecordID;

            this.JobGrpDes.Enabled = EnableMode;
            ...
            else
            {
                ...
                if (this.objJobGrpFactory.IsNew)
                {
        ...
        private void ListSelectionSync()
        {
            ...
            try
            {
                if (objJobGrpFactory.ObjREFJobGrp.JobGrpKey > 0)
                {
                    ...
                    if (tagrdJobGrp.Selected != null)
                        if (tagrdJobGrp.Selected.Cells.Count > 0)
                            if (GFunc.NEInt(tagrdJobGrp.Selected.Cells[0].Row.Cells["JobGrpKey"].Value, 0) == objJobGrpFactory.ObjREFJobGrp.JobGrpKey)
                                return;//already selected.

                    //Select the row on the list to reflect the current value in Header.recordkey
                    UltraGridRow ToSelectRow = this.tagrdJobGrp.Rows.OfType<UltraGridRow>().ToList().Find(
                    row => row.Cells["JobGrpKey"].Text.Equals(objJobGrpFactory.ObjREFJobGrp.JobGrpKey.ToString(), StringComparison.CurrentCultureIgnoreCase));
                    if (ToSelectRow != null)
                    {
                        ToSelectRow.Cells["JobGrpID"].Selected = true;
                        ToSelectRow.Cells["JobGrpID"].Activate();
                    }
                }
                else
                {
                    tagrdJobGrp.Selected.Cells.Clear();
                    tagrdJobGrp.ActiveRow = null;
                }
            }
            catch (TAException tex)
            {
        ...
        private bool New_Process()
        {
            ...
            try
            {
                if (form_CanValidate() == false)
                {
                    ...
                    {
                        this.objJobGrpFactory.IsDirty = false;
                    }
                }

                this.errorProvider1.Clear();

                if (this.objJobGrpFactory.IsDirty)
                {
                    if (Save_Process() == false)
                        return false;
                }

                if (this.objJobGrpFactory.New() == false)
                {                   
                    return false;
                }
                else
        ...
        private bool OpenRecord(int key)
        {
            ...
            try
            {
                if (SECPermUtility.Edit(objJobGrpFactory.PermID, false))
                {
                    if (objJobGrpFactory.GetEdit(key) == false)
                    {
                        if (SysOptionUtility.GetBool("WarnOpenRecordAsReadOnly"))
                        {
                            ...
                            {
                                objJobGrpFactory.GetReadOnly(key);
                            }
                        }
                    }
                }
        ...
        private bool Delete_Process()
        {
            ...
            try
            {
                ...
                if (this.objJobGrpFactory.Delete())
                {
                    this.objJobGrpFactory.New();
                   
                    //Move the cursor position of active row index to upper row
                    if (tagrdJobGrp.ActiveRow.Index > 0)
                        PreRowIndex = tagrdJobGrp.ActiveRow.Index - 1;
            ...
            finally
            {
                this.Refresh_Form(false);
                this.FormLayout();
                if (tagrdJobGrp.Rows.Count > 0)
                {
                    tagrdJobGrp.Rows[PreRowIndex].Selected = true;
                    tagrdJobGrp.Rows[PreRowIndex].Activate();
                }
                this.Cursor = Cursors.Default;
            }
        }//Completed
        private bool Clear_Process()
        {
            ...
            try
            {
                if (GFunc.IsNEZ(this.objJobGrpFactory.ObjREFJobGrp.JobGrpKey))
                {
        ...
        //Grid Events
        private void tagrdJobGrp_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void tagrdJobGrp_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {
            int key = 0;
            try
            {
                ...
                {
                    if (this.tagrdJobGrp.ActiveRow != null && tagrdJobGrp.Selected.Cells.Count > 0)//this line is required to check if user have selected a row in the list
                        key = GFunc.NEInt(tagrdJobGrp.Selected.Cells[0].Row.Cells["JobGrpKey"].Value, 0);
