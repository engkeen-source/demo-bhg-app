...
namespace WinUI
{
    public partial class frmREFJobTask : Form
    {
        #region Local Variables
        private BOLib.REFJobTaskFactory objJobTaskFactory = null;
        ...
        //Initialize
        public frmREFJobTask()
        {
            InitializeComponent();
        }//Completed
        public frmREFJobTask(string id)
        {
            InitializeComponent();
            recordID = id;
        ...
        public frmREFJobTask(GEnum.SystemCode DocCodeKey, DataTable dtHeader, DataSet dsDetail)
        {
        ...
        //Form Events
        private void frmREFJobTask_Load(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;

            try
            {
                this.tagrdJobTask.DisplayLayout.Bands[0].SortedColumns.Clear();
                //Call Initialization
                this.objJobTaskFactory = new BOLib.REFJobTaskFactory(BOLib.GEnum.InstanceMode.Normal);
                if (objJobTaskFactory.GUID <= 0)
                {
                    formClose = true;
                    return;
                }
                ...
                this.objJobTaskFactory.ErrorNotifierHeader_Set += new GVar.UINotifierEvent(this.ErrorNotifier_Set);
                this.objJobTaskFactory.ErrorNotifierHeader_Clear += new GVar.UINotifierEvent(this.ErrorNotifier_Clear);
                ...
                {
                    if (objJobTaskFactory.SetReadOnlyData(_dtHeader, _dsDetail) == false)
                    {
                        formClose = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    else if (formOpenMode == GEnum.formInitMode.Add)
                    {
                        if (canEditRecordID && recordID != string.Empty)
                            this.JobTaskID.SetValueTrigger(recordID, false);
                    }
                }

                //Setup Grid Layout, ShortcutMenu, fill Combo
                GlobalUI.FormGrids_Set(this, (int)objJobTaskFactory.ConstantCodeKey, out ContextMenuSetting);
                ContextMenuSetting += GlobalUI.ContextMenuSetting_GetNew((int)objJobTaskFactory.ConstantCodeKey);
                //GlobalUI.cmnuGlobal_Set(this);
                GlobalUI.Combos_Fill(this, (int)objJobTaskFactory.ConstantCodeKey);

                //Setup List properties
                this.tagrdJobTask.DisplayLayout.Override.AllowDelete = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdJobTask.DisplayLayout.Override.AllowUpdate = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdJobTask.DisplayLayout.Bands[0].Override.ActiveCellAppearance.BackColor = Color.Gold;
        ...
        private void frmREFJobTask_Shown(object sender, EventArgs e)
        {
            //Close Form when FormLoad has encounter errors
            if (formClose)
                this.Close();
        }//Completed
        private void frmREFJobTask_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.formClose && objJobTaskFactory == null)
                return;
            try
            {
                ...
                //Dispose Factory
                if ((bool)this.objJobTaskFactory.Dispose() == false)
            ...
            {
                if (e.Cancel == false)
                    this.objJobTaskFactory.Dispose();
            }
        }//Completed
        private void frmREFJobTask_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void Refresh_Header()
        {
            try
            {
                this.bdsJobTaskDet.DataSource = objJobTaskFactory.ObjREFJobTask;
                this.bdsJobTaskDet.AllowNew = true;
                this.bdsJobTaskDet.ResetBindings(false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void Refresh_GridList()
        {
            try
            {
                ...
                {
                    string listID = GlobalUI.ListSettingID_Get(ContextMenuSetting, "0GF", tagrdJobTask.Name);
                    GlobalUI.Grid_Format(tagrdJobTask, listID, true, false);
                }
                if (this.tagrdJobTask.DisplayLayout.Bands[0].SortedColumns.Count <= 0)
                    this.tagrdJobTask.DisplayLayout.Bands[0].SortedColumns.Add(tagrdJobTask.DisplayLayout.Bands[0].Columns["JobTaskID"], false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        {
            bool EnableMode = !this.objJobTaskFactory.IsReadOnly;
            this.tslReadOnly.Text = !EnableMode ? "Read Only" : string.Empty;

            //Special Condition for RecordID
            this.JobTaskID.Enabled = EnableMode;
            this.JobTaskID.ReadOnly = !canEditRecordID;

            this.JobTaskDes.Enabled = EnableMode;
            ...
            else
            {
                ...
                if (this.objJobTaskFactory.IsNew)
                {
        ...
        private void ListSelectionSync()
        {
            ...
            try
            {
                if (objJobTaskFactory.ObjREFJobTask.JobTaskKey > 0)
                {
                    ...
                    if (tagrdJobTask.Selected != null)
                        if (tagrdJobTask.Selected.Cells.Count > 0)
                            if (GFunc.NEInt(tagrdJobTask.Selected.Cells[0].Row.Cells["JobTaskKey"].Value, 0) == objJobTaskFactory.ObjREFJobTask.JobTaskKey)
                                return;//already selected.

                    //Select the row on the list to reflect the current value in Header.recordkey
                    UltraGridRow ToSelectRow = this.tagrdJobTask.Rows.OfType<UltraGridRow>().ToList().Find(
                    row => row.Cells["JobTaskKey"].Text.Equals(objJobTaskFactory.ObjREFJobTask.JobTaskKey.ToString(), StringComparison.CurrentCultureIgnoreCase));
                    if (ToSelectRow != null)
                    {
                        ToSelectRow.Cells["JobTaskID"].Selected = true;
                        ToSelectRow.Cells["JobTaskID"].Activate();
                    }
                }
                else
                {
                    tagrdJobTask.Selected.Cells.Clear();
                    tagrdJobTask.ActiveRow = null;
                }
            }
            catch (TAException tex)
            {
        ...
        private bool New_Process()
        {
            ...
            try
            {
                if (form_CanValidate() == false)
                {
                    ...
                    {
                        this.objJobTaskFactory.IsDirty = false;
                    }
                }

                this.errorProvider1.Clear();

                if (this.objJobTaskFactory.IsDirty)
                {
                    if (Save_Process() == false)
                        return false;
                }

                if (this.objJobTaskFactory.New() == false)
                {                   
                    return false;
                }
                else
        ...
        private bool Save_Process()
        {
            ...
            try
            {
                ...
                //Perform Saving
                if (this.objJobTaskFactory.Save())
                {
                    GlobalUI.ResetControlDirty(this);
                    return true;
                }
        ...
        private bool OpenRecord(int key)
        {
            ...
            try
            {
                if (SECPermUtility.Edit(objJobTaskFactory.PermID, false))
                {
                    if (objJobTaskFactory.GetEdit(key) == false)
                    {
                        if (SysOptionUtility.GetBool("WarnOpenRecordAsReadOnly"))
                        {
                            ...
                            {
                                objJobTaskFactory.GetReadOnly(key);
                            }
                        }
                    }
                }
        ...
        private bool Delete_Process()
        {
            ...
            try
            {
                ...
                if (this.objJobTaskFactory.Delete())
                {
                    this.objJobTaskFactory.New();
                   
                    //Move the cursor position of active row index to upper row
                    if (tagrdJobTask.ActiveRow.Index > 0)
                        PreRowIndex = tagrdJobTask.ActiveRow.Index - 1;
            ...
            finally
            {
                this.Refresh_Form(false);
                this.FormLayout();
                if (tagrdJobTask.Rows.Count > 0)
                {
                    tagrdJobTask.Rows[PreRowIndex].Selected = true;
                    tagrdJobTask.Rows[PreRowIndex].Activate();
                }
                this.Cursor = Cursors.Default;
            }
        }//Completed
        private bool Clear_Process()
        {
            ...
            try
            {
                if (GFunc.IsNEZ(this.objJobTaskFactory.ObjREFJobTask.JobTaskKey))
                {
        ...
        //Grid Events
        private void tagrdJobTask_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void tagrdJobTask_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {
            int key = 0;
            try
            {
                ...
                {
                    if (this.tagrdJobTask.ActiveRow != null && tagrdJobTask.Selected.Cells.Count > 0)//this line is required to check if user have selected a row in the list
                        key = GFunc.NEInt(tagrdJobTask.Selected.Cells[0].Row.Cells["JobTaskKey"].Value, 0);
