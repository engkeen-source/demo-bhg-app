...
namespace WinUI
{
    public partial class frmREFLoc : Form
    {
        #region Local Variables
        private BOLib.REFLocFactory objLocFactory = null;
        ...
        public frmREFLoc(GEnum.SystemCode DocCodeKey, DataTable dtHeader, DataSet dsDetail)
        {
        ...
        //Form Events
        private void frmREFLoc_Load(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;

            try
            {
                this.tagrdLoc.DisplayLayout.Bands[0].SortedColumns.Clear();
                //Call Initialization
                this.objLocFactory = new BOLib.REFLocFactory(BOLib.GEnum.InstanceMode.Normal);
                if (objLocFactory.GUID <= 0)
                {
                    formClose = true;
                    return;
                }
                ...
                this.objLocFactory.ErrorNotifierHeader_Set += new GVar.UINotifierEvent(this.ErrorNotifier_Set);
                this.objLocFactory.ErrorNotifierHeader_Clear += new GVar.UINotifierEvent(this.ErrorNotifier_Clear);
                ...
                {
                    if (objLocFactory.SetReadOnlyData(_dtHeader, _dsDetail) == false)
                    {
                        formClose = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    else if (formOpenMode == GEnum.formInitMode.Add)
                    {
                        if (canEditRecordID && recordID != string.Empty)
                            this.LocID.SetValueTrigger(recordID, false);
                    }
                }

                //Setup Grid Layout, ShortcutMenu, fill Combo
                GlobalUI.FormGrids_Set(this, (int)objLocFactory.ConstantCodeKey, out ContextMenuSetting);
                ContextMenuSetting += GlobalUI.ContextMenuSetting_GetNew((int)objLocFactory.ConstantCodeKey);
                //GlobalUI.cmnuGlobal_Set(this);
                GlobalUI.Combos_Fill(this, (int)objLocFactory.ConstantCodeKey);

                //Setup List properties
                this.tagrdLoc.DisplayLayout.Override.AllowDelete = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdLoc.DisplayLayout.Override.AllowUpdate = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdLoc.DisplayLayout.Bands[0].Override.ActiveCellAppearance.BackColor = Color.Gold;
        ...
        private void frmREFLoc_Shown(object sender, EventArgs e)
        {
            //Close Form when FormLoad has encounter errors
            if (formClose)
                this.Close();
        }//Completed
        private void frmREFLoc_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.formClose && objLocFactory == null)
                return;
            try
            {
                ...
                //Dispose Factory
                if ((bool)this.objLocFactory.Dispose() == false)
            ...
            {
                if (e.Cancel == false)
                    this.objLocFactory.Dispose();
            }
        }//Completed
        private void frmREFLoc_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void Refresh_Header()
        {
            try
            {
                this.bdsLocDet.DataSource = objLocFactory.ObjREFLoc;
                this.bdsLocDet.AllowNew = true;
                this.bdsLocDet.ResetBindings(false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void Refresh_GridList()
        {
            try
            {
                ...
                {
                    string listID = GlobalUI.ListSettingID_Get(ContextMenuSetting, "0GF", tagrdLoc.Name);
                    GlobalUI.Grid_Format(tagrdLoc, listID, true, false);
                }
                if (this.tagrdLoc.DisplayLayout.Bands[0].SortedColumns.Count <= 0)
                    this.tagrdLoc.DisplayLayout.Bands[0].SortedColumns.Add(tagrdLoc.DisplayLayout.Bands[0].Columns["LocID"], false);
        ...
        {
            bool EnableMode = !this.objLocFactory.IsReadOnly;
            this.tslReadOnly.Text = !EnableMode ? "Read Only" : string.Empty;

            //Special Condition for RecordID
            this.LocID.Enabled = EnableMode;
            this.LocID.ReadOnly = !canEditRecordID;

            this.LocDes.Enabled = EnableMode;
        ...
        private void ListSelectionSync()
        {
            ...
            try
            {
                if (objLocFactory.ObjREFLoc.LocKey > 0)
                {
                    ...
                    if (tagrdLoc.Selected != null)
                        if (tagrdLoc.Selected.Cells.Count > 0)
                            if (GFunc.NEInt(tagrdLoc.Selected.Cells[0].Row.Cells["LocKey"].Value, 0) == objLocFactory.ObjREFLoc.LocKey)
                                return;//already selected.

                    //Select the row on the list to reflect the current value in Header.recordkey
                    UltraGridRow ToSelectRow = this.tagrdLoc.Rows.OfType<UltraGridRow>().ToList().Find(
                    row => row.Cells["LocKey"].Text.Equals(objLocFactory.ObjREFLoc.LocKey.ToString(), StringComparison.CurrentCultureIgnoreCase));
                    if (ToSelectRow != null)
                    {
                        ToSelectRow.Cells["LocID"].Selected = true;
                        ToSelectRow.Cells["LocID"].Activate();
                    }
                }
                else
                {
                    tagrdLoc.Selected.Cells.Clear();
                    tagrdLoc.ActiveRow = null;
                }
            }
            catch (TAException tex)
            {
        ...
        private bool New_Process()
        {
            ...
            try
            {
                if (form_CanValidate() == false)
                {
                    ...
                    {
                        this.objLocFactory.IsDirty = false;
                    }
                }

                this.errorProvider1.Clear();

                if (this.objLocFactory.IsDirty)
                {
                    if (Save_Process() == false)
                        return false;
                }

                if (this.objLocFactory.New() == false)
                {                    
                    return false;
                }
                else
        ...
        private bool OpenRecord(int key)
        {
            ...
            try
            {
                if (SECPermUtility.Edit(objLocFactory.PermID, false))
                {
                    if (objLocFactory.GetEdit(key) == false)
                    {
        ...
        private bool Delete_Process()
        {
            ...
            try
            {
                ...
                if (this.objLocFactory.Delete())
                {
                    this.objLocFactory.New();
                   
                    //Move the cursor position of active row index to upper row
                    if (tagrdLoc.ActiveRow.Index > 0)
                        PreRowIndex = tagrdLoc.ActiveRow.Index - 1;
                    GlobalUI.ResetControlDirty(this);
                    return true;
                }
                else
            ...
            finally
            {
                ...
                if (tagrdLoc.Rows.Count > 0)
                {
                    tagrdLoc.Rows[PreRowIndex].Selected = true;
                    tagrdLoc.Rows[PreRowIndex].Activate();
                }
                this.Cursor = Cursors.Default;
            }
        }//Completed
        private bool Clear_Process()
        {
            ...
            try
            {
                if (GFunc.IsNEZ(this.objLocFactory.ObjREFLoc.LocKey))
                {
        ...
        //Grid Events
        private void tagrdLoc_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void tagrdLoc_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {
            int key = 0;
            try
            {
                ...
                {
                    if (this.tagrdLoc.ActiveRow != null && tagrdLoc.Selected.Cells.Count > 0)//this line is required to check if user have selected a row in the list
                        key = GFunc.NEInt(tagrdLoc.Selected.Cells[0].Row.Cells["LocKey"].Value, 0);
