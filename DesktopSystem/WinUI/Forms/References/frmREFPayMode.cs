...
namespace WinUI
{
    public partial class frmREFPayMode : Form
    {
        #region Local Variables
        private BOLib.REFPayModeFactory objPayModeFactory = null;
        ...
        //Initialize
        public frmREFPayMode()
        {
            InitializeComponent();
        }//Completed
        public frmREFPayMode(string id)
        {
            InitializeComponent();
            recordID = id;
        ...
        public frmREFPayMode(GEnum.SystemCode DocCodeKey, DataTable dtHeader, DataSet dsDetail)
        {
        ...
        //Form Events
        private void frmREFPayMode_Load(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;

            try
            {
                this.tagrdPayMode.DisplayLayout.Bands[0].SortedColumns.Clear();
                //Call Initialization
                this.objPayModeFactory = new BOLib.REFPayModeFactory(BOLib.GEnum.InstanceMode.Normal);
                if (objPayModeFactory.GUID <= 0)
                {
                    formClose = true;
                    return;
                }
                ...
                this.objPayModeFactory.ErrorNotifierHeader_Set += new GVar.UINotifierEvent(this.ErrorNotifier_Set);
                this.objPayModeFactory.ErrorNotifierHeader_Clear += new GVar.UINotifierEvent(this.ErrorNotifier_Clear);
                ...
                {
                    if (objPayModeFactory.SetReadOnlyData(_dtHeader, _dsDetail) == false)
                    {
                        formClose = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    else if (formOpenMode == GEnum.formInitMode.Add)
                    {
                        if (canEditRecordID && recordID != string.Empty)
                            this.PayModeID.SetValueTrigger(recordID, false);
                    }
                }

                //Setup Grid Layout, ShortcutMenu, fill Combo
                GlobalUI.FormGrids_Set(this, (int)objPayModeFactory.ConstantCodeKey, out ContextMenuSetting);
                ContextMenuSetting += GlobalUI.ContextMenuSetting_GetNew((int)objPayModeFactory.ConstantCodeKey);
                //GlobalUI.cmnuGlobal_Set(this);
                GlobalUI.Combos_Fill(this, (int)objPayModeFactory.ConstantCodeKey);

                //Setup List properties
                this.tagrdPayMode.DisplayLayout.Override.AllowDelete = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdPayMode.DisplayLayout.Override.AllowUpdate = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdPayMode.DisplayLayout.Bands[0].Override.ActiveCellAppearance.BackColor = Color.Gold;
        ...
        private void frmREFPayMode_Shown(object sender, EventArgs e)
        {
            //Close Form when FormLoad has encounter errors
            if (formClose)
                this.Close();
        }//Completed
        private void frmREFPayMode_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.formClose && objPayModeFactory == null)
                return;
            ...
            try
            {
                ...
                //Dispose Factory
                if ((bool)this.objPayModeFactory.Dispose() == false)
            ...
            {
                if (e.Cancel == false)
                    this.objPayModeFactory.Dispose();
            }
        }//Completed
        private void frmREFPayMode_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void Refresh_Header()
        {
            try
            {
                this.bdsPayModeDet.DataSource = objPayModeFactory.ObjREFPayMode;
                this.bdsPayModeDet.AllowNew = true;
                this.bdsPayModeDet.ResetBindings(false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void Refresh_GridList()
        {
            try
            {
                ...
                {
                    string listID = GlobalUI.ListSettingID_Get(ContextMenuSetting, "0GF", tagrdPayMode.Name);
                    GlobalUI.Grid_Format(tagrdPayMode, listID, true, false);
                }
                if (this.tagrdPayMode.DisplayLayout.Bands[0].SortedColumns.Count <= 0)
                    this.tagrdPayMode.DisplayLayout.Bands[0].SortedColumns.Add(tagrdPayMode.DisplayLayout.Bands[0].Columns["PayModeID"], false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        {
            bool EnableMode = !this.objPayModeFactory.IsReadOnly;
            this.tslReadOnly.Text = !EnableMode ? "Read Only" : string.Empty;

            //Special Condition for RecordID
            this.PayModeID.Enabled = EnableMode;
            this.PayModeID.ReadOnly = !canEditRecordID;

            this.PayModeDes.Enabled = EnableMode;
            ...
            else
            {
                ...
                if (this.objPayModeFactory.IsNew)
                {
        ...
        private void ListSelectionSync()
        {
            ...
            try
            {
                if (objPayModeFactory.ObjREFPayMode.PayModeKey > 0)
                {
                    ...
                    if (tagrdPayMode.Selected != null)
                        if (tagrdPayMode.Selected.Cells.Count > 0)
                            if (GFunc.NEInt(tagrdPayMode.Selected.Cells[0].Row.Cells["PayModeKey"].Value, 0) == objPayModeFactory.ObjREFPayMode.PayModeKey)
                                return;//already selected.

                    //Select the row on the list to reflect the current value in Header.recordkey
                    UltraGridRow ToSelectRow = this.tagrdPayMode.Rows.OfType<UltraGridRow>().ToList().Find(
                    row => row.Cells["PayModeKey"].Text.Equals(objPayModeFactory.ObjREFPayMode.PayModeKey.ToString(), StringComparison.CurrentCultureIgnoreCase));
                    if (ToSelectRow != null)
                    {
                        ToSelectRow.Cells["PayModeID"].Selected = true;
                        ToSelectRow.Cells["PayModeID"].Activate();
                    }
                }
                else
                {
                    tagrdPayMode.Selected.Cells.Clear();
                    tagrdPayMode.ActiveRow = null;
                }
            }
            catch (TAException tex)
            {
        ...
        private bool New_Process()
        {
            ...
            try
            {
                if (form_CanValidate() == false)
                {
                    ...
                    {
                        this.objPayModeFactory.IsDirty = false;
                    }
                }

                this.errorProvider1.Clear();

                if (this.objPayModeFactory.IsDirty)
                {
                    if (Save_Process() == false)
                        return false;
                }

                if (this.objPayModeFactory.New() == false)
                {                   
                    return false;
                }
                else
        ...
        private bool Save_Process()
        {
            ...
            try
            {
                ...
                //Perform Saving
                if (this.objPayModeFactory.Save())
                {
                    GlobalUI.ResetControlDirty(this);
                    return true;
                }
        ...
        private bool OpenRecord(int key)
        {
            ...
            try
            {
                if (SECPermUtility.Edit(objPayModeFactory.PermID, false))
                {
                    if (objPayModeFactory.GetEdit(key) == false)
                    {
                        if (SysOptionUtility.GetBool("WarnOpenRecordAsReadOnly"))
                        {
                            ...
                            {
                                objPayModeFactory.GetReadOnly(key);
                            }
                        }
                    }
                }
        ...
        private bool Delete_Process()
        {
            ...
            try
            {
                ...
                if (this.objPayModeFactory.Delete())
                {
                    this.objPayModeFactory.New();
                    
                    //Move the cursor position of active row index to upper row
                    if (tagrdPayMode.ActiveRow.Index > 0)
                        PreRowIndex = tagrdPayMode.ActiveRow.Index - 1;
            ...
            {
                this.Refresh_Form(false);
                if (tagrdPayMode.Rows.Count > 0)
                {
                    tagrdPayMode.Rows[PreRowIndex].Selected = true;
                    tagrdPayMode.Rows[PreRowIndex].Activate();
                }
                this.Cursor = Cursors.Default;
            }
        }//Completed
        private bool Clear_Process()
        {
            ...
            try
            {
                if (GFunc.IsNEZ(this.objPayModeFactory.ObjREFPayMode.PayModeKey))
                {
        ...
        //Grid Events
        private void tagrdPayMode_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void tagrdPayMode_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {
            int key = 0;
            try
            {
                ...
                {
                    if (this.tagrdPayMode.ActiveRow != null && tagrdPayMode.Selected.Cells.Count > 0)//this line is required to check if user have selected a row in the list
                        key = GFunc.NEInt(tagrdPayMode.Selected.Cells[0].Row.Cells["PayModeKey"].Value, 0);
