...
namespace WinUI
{
    public partial class frmREFTerritory : Form
    {
        #region Local Variables
        private BOLib.REFTerritoryFactory objTerritoryFactory = null;
        ...
        //Initialize
        public frmREFTerritory()
        {
            InitializeComponent();
        }//Completed
        public frmREFTerritory(string id)
        {
            InitializeComponent();
            recordID = id;
        ...
        public frmREFTerritory(int Key)
        {
        ...
        public frmREFTerritory(GEnum.SystemCode DocCodeKey, DataTable dtHeader, DataSet dsDetail)
        {
        ...
        //Form Events
        private void frmREFTerritory_Load(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;

            try
            {
                this.tagrdTerritory.DisplayLayout.Bands[0].SortedColumns.Clear();
                //Call Initialization
                this.objTerritoryFactory = new BOLib.REFTerritoryFactory(BOLib.GEnum.InstanceMode.Normal);
                if (objTerritoryFactory.GUID <= 0)
                {
                    formClose = true;
                    return;
                }
                ...
                this.objTerritoryFactory.ErrorNotifierHeader_Set += new GVar.UINotifierEvent(this.ErrorNotifier_Set);
                this.objTerritoryFactory.ErrorNotifierHeader_Clear += new GVar.UINotifierEvent(this.ErrorNotifier_Clear);
                ...
                {
                    if (objTerritoryFactory.SetReadOnlyData(_dtHeader, _dsDetail) == false)
                    {
                        formClose = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    else if (formOpenMode == GEnum.formInitMode.Add)
                    {
                        if (canEditRecordID && recordID != string.Empty)
                            this.TerritoryID.SetValueTrigger(recordID, false);
                    }
                }

                //Setup Grid Layout, ShortcutMenu, fill Combo
                GlobalUI.FormGrids_Set(this, (int)objTerritoryFactory.ConstantCodeKey, out ContextMenuSetting);
                ContextMenuSetting += GlobalUI.ContextMenuSetting_GetNew((int)objTerritoryFactory.ConstantCodeKey);
                //GlobalUI.cmnuGlobal_Set(this);
                GlobalUI.Combos_Fill(this, (int)objTerritoryFactory.ConstantCodeKey);

                //Setup List properties
                this.tagrdTerritory.DisplayLayout.Override.AllowDelete = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdTerritory.DisplayLayout.Override.AllowUpdate = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdTerritory.DisplayLayout.Bands[0].Override.ActiveCellAppearance.BackColor = Color.Gold;
        ...
        private void frmREFTerritory_Shown(object sender, EventArgs e)
        {
            //Close Form when FormLoad has encounter errors
            if (formClose)
                this.Close();
        }//Completed
        private void frmREFTerritory_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.formClose && objTerritoryFactory == null)
                return;
            ...
            try
            {
                ...
                //Dispose Factory
                if ((bool)this.objTerritoryFactory.Dispose() == false)
            ...
            {
                if (e.Cancel == false)
                    this.objTerritoryFactory.Dispose();
            }
        }//Completed
        private void frmREFTerritory_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void Refresh_Header()
        {
            try
            {
                this.bdsTerritoryDet.DataSource = objTerritoryFactory.ObjREFTerritory;
                this.bdsTerritoryDet.AllowNew = true;
                this.bdsTerritoryDet.ResetBindings(false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void Refresh_GridList()
        {
            try
            {
                ...
                {
                    string listID = GlobalUI.ListSettingID_Get(ContextMenuSetting, "0GF", tagrdTerritory.Name);
                    GlobalUI.Grid_Format(tagrdTerritory, listID, true, false);
                }
                if (this.tagrdTerritory.DisplayLayout.Bands[0].SortedColumns.Count <= 0)
                    this.tagrdTerritory.DisplayLayout.Bands[0].SortedColumns.Add(tagrdTerritory.DisplayLayout.Bands[0].Columns["TerritoryID"], false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        {
            bool EnableMode = !this.objTerritoryFactory.IsReadOnly;
            this.tslReadOnly.Text = !EnableMode ? "Read Only" : string.Empty;

            //Special Condition for RecordID
            this.TerritoryID.Enabled = EnableMode;
            this.TerritoryID.ReadOnly = !canEditRecordID;

            this.TerritoryDes.Enabled = EnableMode;
            ...
            else
            {
                ...
                if (this.objTerritoryFactory.IsNew)
                {
        ...
        private void ListSelectionSync()
        {
            ...
            try
            {
                if (objTerritoryFactory.ObjREFTerritory.TerritoryKey > 0)
                {
                    ...
                    if (tagrdTerritory.Selected != null)
                        if (tagrdTerritory.Selected.Cells.Count > 0)
                            if (GFunc.NEInt(tagrdTerritory.Selected.Cells[0].Row.Cells["TerritoryKey"].Value, 0) == objTerritoryFactory.ObjREFTerritory.TerritoryKey)
                                return;//already selected.

                    //Select the row on the list to reflect the current value in Header.recordkey
                    UltraGridRow ToSelectRow = this.tagrdTerritory.Rows.OfType<UltraGridRow>().ToList().Find(
                    row => row.Cells["TerritoryKey"].Text.Equals(objTerritoryFactory.ObjREFTerritory.TerritoryKey.ToString(), StringComparison.CurrentCultureIgnoreCase));
                    if (ToSelectRow != null)
                    {
                        ToSelectRow.Cells["TerritoryID"].Selected = true;
                        ToSelectRow.Cells["TerritoryID"].Activate();
                    }
                }
                else
                {
                    tagrdTerritory.Selected.Cells.Clear();
                    tagrdTerritory.ActiveRow = null;
                }
            }
            catch (TAException tex)
            {
        ...
        private bool New_Process()
        {
            ...
            try
            {
                if (form_CanValidate() == false)
                {
                    ...
                    {
                        this.objTerritoryFactory.IsDirty = false;
                    }
                }

                this.errorProvider1.Clear();

                if (this.objTerritoryFactory.IsDirty)
                {
                    if (Save_Process() == false)
                        return false;
                }

                if (this.objTerritoryFactory.New() == false)
                {                   
                    return false;
                }
                else
        ...
        private bool SaveChanges()
        {
            try
            {
                ...
                if (objTerritoryFactory.IsDirty)
                {
                    GEnum.MsgBoxButton btnSelect;
        ...
        private bool Save_Process()
        {
            ...
            try
            {
                ...
                //Perform Saving
                if (this.objTerritoryFactory.Save())
                {
                    GlobalUI.ResetControlDirty(this);
                    return true;
                }
        ...
        private bool OpenRecord(int key)
        {
            ...
            try
            {
                if (SECPermUtility.Edit(objTerritoryFactory.PermID, false))
                {
                    if (objTerritoryFactory.GetEdit(key) == false)
                    {
                        if (SysOptionUtility.GetBool("WarnOpenRecordAsReadOnly"))
                        {
                            ...
                            {
                                objTerritoryFactory.GetReadOnly(key);
                            }
                        }
                    }
                }
        ...
        private bool Delete_Process()
        {
            ...
            try
            {
                ...
                if (this.objTerritoryFactory.Delete())
                {
                    this.objTerritoryFactory.New();
                    
                    //Move the cursor position of active row index to upper row
                    if (tagrdTerritory.ActiveRow.Index > 0)
                        PreRowIndex = tagrdTerritory.ActiveRow.Index - 1;
            ...
            finally
            {
                this.Refresh_Form(false);
                this.FormLayout();
                if (tagrdTerritory.Rows.Count > 0)
                {
                    tagrdTerritory.Rows[PreRowIndex].Selected = true;
                    tagrdTerritory.Rows[PreRowIndex].Activate();
                }
                this.Cursor = Cursors.Default;
            }
        }//Completed
        private bool Clear_Process()
        {
            ...
            try
            {
                if (GFunc.IsNEZ(this.objTerritoryFactory.ObjREFTerritory.TerritoryKey))
                {
                    ...
                    if (this.objTerritoryFactory.New())
                    {                        
        ...
        //Grid Events
        private void tagrdTerritory_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void tagrdTerritory_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {
            int key = 0;
            try
            {
                ...
                {
                    if (this.tagrdTerritory.ActiveRow != null && tagrdTerritory.Selected.Cells.Count > 0)//this line is required to check if user have selected a row in the list
                        key = GFunc.NEInt(tagrdTerritory.Selected.Cells[0].Row.Cells["TerritoryKey"].Value, 0);
