...
namespace WinUI
{
    public partial class frmPageSetupRpx : Form
    {
        #region Variables              
      
        DataDynamics.ActiveReports.ActiveReport Report_Source;       
        ...
        //Constructure
        public frmPageSetupRpx()
        {
            InitializeComponent();
        }
     
        public frmPageSetupRpx(DataDynamics.ActiveReports.ActiveReport Parm_ReportSource)
        {
            InitializeComponent();
        ...
        //Form Events
        private void frmPageSetupRpx_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Report_Source.Document.Printer.PrinterName!= "")
                {  
                    pDlg.PrinterSettings= Report_Source.Document.Printer.PrinterSettings;
        ...
        private void SetMargins()
        {
            ...
            DataDynamics.ActiveReports.PageSettings tmpPageSettings = this.Report_Source.PageSettings;
            TopMarginValue.Value = DataDynamics.ActiveReports.ActiveReport.InchToCm((float)(System.Math.Round(this.Report_Source.PageSettings.Margins.Top, 2)));// System.Math.Round(this.Report_Source.PageSettings.Margins.Top * 2.54F, 2);
            LeftMarginValue.Value = DataDynamics.ActiveReports.ActiveReport.InchToCm((float)(System.Math.Round(this.Report_Source.PageSettings.Margins.Left, 2)));// System.Math.Round(this.Report_Source.PageSettings.Margins.Left * 2.54F, 2);
            RightMarginValue.Value = DataDynamics.ActiveReports.ActiveReport.InchToCm((float)(System.Math.Round(this.Report_Source.PageSettings.Margins.Right, 2)));// System.Math.Round(this.Report_Source.PageSettings.Margins.Right * 2.54F, 2);
            BottomMarginValue.Value = DataDynamics.ActiveReports.ActiveReport.InchToCm((float)(System.Math.Round(this.Report_Source.PageSettings.Margins.Bottom, 2)));// System.Math.Round(this.Report_Source.PageSettings.Margins.Bottom * 2.54F, 2);
        ...
        private void frmPageSetupRpx_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void btnOK_Click(object sender, EventArgs e)
        {
            try
            {
                this.Report_Source.Document.Printer.PrinterName = pDlg.PrinterSettings.PrinterName;
                this.Report_Source.Document.Printer.PrinterSettings = pDlg.PrinterSettings;
                this.Report_Source.Document.Printer.DefaultPageSettings = pDlg.PrinterSettings.DefaultPageSettings;

                this.Report_Source.PageSettings.PaperKind = pDlg.PrinterSettings.DefaultPageSettings.PaperSize.Kind;
                this.Report_Source.PageSettings.PaperSource = pDlg.PrinterSettings.DefaultPageSettings.PaperSource.Kind;  

               // if (pDlg.PrinterSettings.DefaultPageSettings.PaperSize.Kind == PaperKind.Custom)
                    this.Report_Source.PageSettings.PaperName = pDlg.PrinterSettings.DefaultPageSettings.PaperSize.PaperName;

               // if (pDlg.PrinterSettings.DefaultPageSettings.PaperSource.Kind == PaperSourceKind.Custom || pDlg.PrinterSettings.DefaultPageSettings.PaperSource.Kind == PaperSourceKind.Manual)
                    this.Report_Source.PageSettings.PaperSourceName = pDlg.PrinterSettings.DefaultPageSettings.PaperSource.SourceName;                

                this.Report_Source.PageSettings.Orientation = pDlg.PrinterSettings.DefaultPageSettings.Landscape? 
                    DataDynamics.ActiveReports.Document.PageOrientation.Landscape: DataDynamics.ActiveReports.Document.PageOrientation.Portrait ;                
                
                this.Report_Source.PageSettings.Margins.Top =(float)(Convert.ToDecimal(TopMarginValue.Text)/2.54M);
                this.Report_Source.PageSettings.Margins.Left = (float)Convert.ToDecimal(LeftMarginValue.Text) / 2.54F;
                this.Report_Source.PageSettings.Margins.Bottom = (float)Convert.ToDecimal(BottomMarginValue.Text) / 2.54F;
                this.Report_Source.PageSettings.Margins.Right = (float)Convert.ToDecimal(RightMarginValue.Text) / 2.54F;
        ...
        private void SetPrinterPaperSizesAndPaperSource(bool GetPapersizeFromRpt)
        {
            try
            {
                ...
                foreach (System.Drawing.Printing.PaperSize item in pDlg.PrinterSettings.PaperSizes)
                {
                    ...
                    {
                        if (item.Kind == Report_Source.Document.Printer.DefaultPageSettings.PaperSize.Kind)
                        {
                            if (Report_Source.PageSettings.PaperKind == PaperKind.Custom)
                            {
                                if (Report_Source.PageSettings.PaperName == item.PaperName)
                                {                                    
                                    pz = item;
                                    found = true;
                                }
                ...
                else if (dtPaperSize.Rows.Count > 0)
                {
                ...
                foreach (System.Drawing.Printing.PaperSource item in pDlg.PrinterSettings.PaperSources)
                {
                    ...
                    if (GetPapersizeFromRpt && !found)
                    {
                        if ((int)item.Kind == (int)Report_Source.PageSettings.PaperSource)
                        {
                            if (Report_Source.PageSettings.PaperSource == PaperSourceKind.Custom)
                            {
                                if (Report_Source.PageSettings.PaperSourceName == item.SourceName)
                                {
                                    ps = item;
                                    found = true;
                                }
