...
namespace WinUI
{
    public partial class frmMSTItmKittAssBal : Form
    {
        ...
        private void tagrdList_InitializeRow(object sender, InitializeRowEventArgs e)
        {
            if (e.Row.Cells.Exists("ItemStatus"))
            {
                ...
                {
                    e.Row.Cells["ItemStatus"].Appearance.Image = global::WinUI.Properties.Resources.green_border_14;
                }
            ...
            if (e.Row.Cells.Exists("OrderQty_ToKitt") && GFunc.NEDec(e.Row.Cells["OrderQty_ToKitt"].Value, 0) != 0)
            {
                e.Row.Cells["OrderQty_ToKitt"].Appearance.Image = global::WinUI.Properties.Resources.orange_14;
            }
        ...
        private void tagrdList_DoubleClickRow(object sender, DoubleClickRowEventArgs e)
        {
            try
            {
                ...
                switch (CheckCol)
                {
                    ...
                    case "OrderQty_ToKitt":
                    case "OrderQty_ToExportDO":
        ...
        private void Grid_Rebind()
        {
            try
            {
                ...
                dtMstItmBal = GFunc.ExecuteProc("MstItm_KittAssBalGet", parmList);

                tagrdList.DataSource = dtMstItmBal;
                tagrdList.DataBind();

                var rows = dtMstItmBal.AsEnumerable();
                int allCount = rows.Count(r => r.Field<int>("ItmType") == 250);
                int greenCount = rows.Count(r => r.Field<string>("ItemStatus") == "green");                               
                int redCount = rows.Count(r => r.Field<string>("ItemStatus") == "red");
                int orangeCount = rows.Count(r => GFunc.NEDec(r["OrderQty_ToExportDO"], 0) != 0);
                int blueCount = rows.Count(r => r.Field<string>("ItemStatus") == "blue");

                chkAll.Text = "All Active Assembly (" + allCount.ToString("N0") + ")";
                chkPreKitt.Text = "Pre-Kitted Assembly (" + greenCount.ToString("N0") + ")";
                chkProgress.Text = "In Progress (" + orangeCount.ToString("N0") + ")";
                chkComponent.Text = "Kitted Components (" + blueCount.ToString("N0") + ")";
                DisplayRowCount();
            }
        ...
        private void CheckedValueFilter()
        {
            try
            {               
                frmMain.gfrmMain.SetNotifyStatus("Loading.... Please wait.");
                ...
                if (chkAll.Checked)
                    filters.Add("ItmType = 250");

                if (chkComponent.Checked)
                    filters.Add("ItmType = 100");

                if (chkPreKitt.Checked)
                    filters.Add("ItmType = 250 AND GoodsInvnQty <> 0");

                if (chkProgress.Checked)
                    filters.Add("ItmType = 250 AND OrderQty_ToExportDO <> 0");
        ...
        private void ColumnHide(bool HiddenTrue_Assembly)
        {
            tagrdList.DisplayLayout.Bands[0].Columns["BossQty"].Hidden = HiddenTrue_Assembly;
            tagrdList.DisplayLayout.Bands[0].Columns["CrossChecked_AvailableBossQty"].Hidden = HiddenTrue_Assembly;
            tagrdList.DisplayLayout.Bands[0].Columns["Untally_Qty"].Hidden = HiddenTrue_Assembly;
            tagrdList.DisplayLayout.Bands[0].Columns["KittedQty_Standby"].Hidden = HiddenTrue_Assembly;
            tagrdList.DisplayLayout.Bands[0].Columns["PickedQty_ToExportPCN"].Hidden = HiddenTrue_Assembly;
            tagrdList.DisplayLayout.Bands[0].Columns["ReceivedQty_ToExportPD"].Hidden = HiddenTrue_Assembly;            
            tagrdList.DisplayLayout.Bands[0].Columns["OrderQty_ToExportDO"].Hidden = !HiddenTrue_Assembly;
            tagrdList.DisplayLayout.Bands[0].Columns["OrderQty_ToKitt"].Hidden = !HiddenTrue_Assembly;
        }
        private void chkComponent_CheckedChanged(object sender, EventArgs e)
        {
            if (chkComponent.Checked)
            {
                chkAll.CheckedValue = false;
                chkProgress.CheckedValue = false;
                chkPreKitt.CheckedValue = false;
                chkComponent.CheckedValue = true;
                CheckedValueFilter();
                ColumnHide(false);
                //GlobalUI.FormGrids_Set(this, 23101, out ContextMenuSetting);
            }
        ...
        {
            chkComponent.CheckedValue = false;
            CheckedValueFilter();
            ColumnHide(true);
            //GlobalUI.FormGrids_Set(this, 23100, out ContextMenuSetting);
