...
namespace WinUI
{
    public partial class frmREFColor : Form
    {
        #region Local Variables
        private BOLib.REFColorFactory objColorFactory = null;
        ...
        public frmREFColor(GEnum.SystemCode DocCodeKey, DataTable dtHeader, DataSet dsDetail)
        {
        ...
        //Form Events
        private void frmREFColor_Load(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;

            try
            {
                this.tagrdColor.DisplayLayout.Bands[0].SortedColumns.Clear();
                //Call Initialization
                this.objColorFactory = new BOLib.REFColorFactory(BOLib.GEnum.InstanceMode.Normal);
                if (objColorFactory.GUID <= 0)
                {
                    formClose = true;
                    return;
                }
                ...
                this.objColorFactory.ErrorNotifierHeader_Set += new GVar.UINotifierEvent(this.ErrorNotifier_Set);
                this.objColorFactory.ErrorNotifierHeader_Clear += new GVar.UINotifierEvent(this.ErrorNotifier_Clear);
                ...
                {
                    if (objColorFactory.SetReadOnlyData(_dtHeader, _dsDetail) == false)
                    {
                        formClose = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    else if (formOpenMode == GEnum.formInitMode.Add)
                    {
                        if (canEditRecordID && recordID != string.Empty)
                            this.ColorID.SetValueTrigger(recordID, false);
                    }
                }

                //Setup Grid Layout, ShortcutMenu, fill Combo
                GlobalUI.FormGrids_Set(this, (int)objColorFactory.ConstantCodeKey, out ContextMenuSetting);
                ContextMenuSetting += GlobalUI.ContextMenuSetting_GetNew((int)objColorFactory.ConstantCodeKey);
                //GlobalUI.cmnuGlobal_Set(this);
                GlobalUI.Combos_Fill(this, (int)objColorFactory.ConstantCodeKey);

                //Setup List properties
                this.tagrdColor.DisplayLayout.Override.AllowDelete = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdColor.DisplayLayout.Override.AllowUpdate = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdColor.DisplayLayout.Bands[0].Override.ActiveCellAppearance.BackColor = Color.Gold;
        ...
        private void frmREFColor_Shown(object sender, EventArgs e)
        {
            //Close Form when FormLoad has encounter errors
            if (formClose)
                this.Close();
        }//Completed
        private void frmREFColor_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.formClose && objColorFactory == null)
                return;
            try
            {
                ...
                //Dispose Factory
                if ((bool)this.objColorFactory.Dispose() == false)
            ...
            {
                if (e.Cancel == false)
                    this.objColorFactory.Dispose();
            }
        }//Completed
        private void frmREFColor_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void Refresh_Header()
        {
            try
            {
                this.bdsColorDet.DataSource = objColorFactory.ObjREFColor;
                this.bdsColorDet.AllowNew = true;
                this.bdsColorDet.ResetBindings(false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void Refresh_GridList()
        {
            ...
            {                
                ...
                {
                    string listID = GlobalUI.ListSettingID_Get(ContextMenuSetting, "0GF", tagrdColor.Name);
                    GlobalUI.Grid_Format(tagrdColor, listID, true, false);
                }
                if (this.tagrdColor.DisplayLayout.Bands[0].SortedColumns.Count <= 0)
                    this.tagrdColor.DisplayLayout.Bands[0].SortedColumns.Add(tagrdColor.DisplayLayout.Bands[0].Columns["ColorID"], false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        {
            bool EnableMode = !this.objColorFactory.IsReadOnly;
            this.tslReadOnly.Text = !EnableMode ? "Read Only" : string.Empty;

            //Special Condition for RecordID
            this.ColorID.Enabled = EnableMode;
            this.ColorID.ReadOnly = !canEditRecordID;

            this.ColorDes.Enabled = EnableMode;
        ...
        private void ListSelectionSync()
        {
            ...
            try
            {
                if (objColorFactory.ObjREFColor.ColorKey > 0)
                {
                    ...
                    if (tagrdColor.Selected != null)
                        if (tagrdColor.Selected.Cells.Count > 0)
                            if (GFunc.NEInt(tagrdColor.Selected.Cells[0].Row.Cells["ColorKey"].Value, 0) == objColorFactory.ObjREFColor.ColorKey)
                                return;//already selected.

                    //Select the row on the list to reflect the current value in Header.recordkey
                    UltraGridRow ToSelectRow = this.tagrdColor.Rows.OfType<UltraGridRow>().ToList().Find(
                    row => row.Cells["ColorKey"].Text.Equals(objColorFactory.ObjREFColor.ColorKey.ToString(), StringComparison.CurrentCultureIgnoreCase));
                    if (ToSelectRow != null)
                    {
                        ToSelectRow.Cells["ColorID"].Selected = true;
                        ToSelectRow.Cells["ColorID"].Activate();
                    }
                }
                else
                {
                    tagrdColor.Selected.Cells.Clear();
                    tagrdColor.ActiveRow = null;
                }
            }
            catch (TAException tex)
            {
        ...
        private bool New_Process()
        {
            ...
            try
            {
                if (form_CanValidate() == false)
                {
                    ...
                    {
                        this.objColorFactory.IsDirty = false;
                    }
                }

                this.errorProvider1.Clear();

                if (this.objColorFactory.IsDirty)
                {
                    if (Save_Process() == false)
                        return false;
                }

                if (this.objColorFactory.New() == false)
                {                   
                    return false;
                }
                else
        ...
        private bool OpenRecord(int key)
        {
            ...
            try
            {
                if (SECPermUtility.Edit(objColorFactory.PermID, false))
                {
                    if (objColorFactory.GetEdit(key) == false)
                    {
                        if (SysOptionUtility.GetBool("WarnOpenRecordAsReadOnly"))
                        {
                            ...
                            {
                                objColorFactory.GetReadOnly(key);
                            }
                        }
                    }
                }
        ...
        private bool Delete_Process()
        {
            ...
            try
            {
                ...
                if (this.objColorFactory.Delete())
                {
                    this.objColorFactory.New();
        ...
        private bool Clear_Process()
        {
            ...
            try
            {
                if (GFunc.IsNEZ(this.objColorFactory.ObjREFColor.ColorKey))
                {
        ...
        //Grid Events
        private void tagrdColor_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void tagrdColor_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {
            int key = 0;
            try
            {
                ...
                {
                    if (this.tagrdColor.ActiveRow != null && tagrdColor.Selected.Cells.Count > 0)//this line is required to check if user have selected a row in the list
                        key = GFunc.NEInt(tagrdColor.Selected.Cells[0].Row.Cells["ColorKey"].Value, 0);
