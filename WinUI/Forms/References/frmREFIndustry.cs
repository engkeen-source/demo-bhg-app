...
namespace WinUI
{
    public partial class frmREFIndustry : Form
    {
        #region Local Variables
        private BOLib.REFIndustryFactory objIndustryFactory = null;
        ...
        //Initialize
        public frmREFIndustry()
        {
            InitializeComponent();
        }//Completed
        public frmREFIndustry(string id)
        {
            InitializeComponent();
            recordID = id;
        ...
        public frmREFIndustry(GEnum.SystemCode DocCodeKey, DataTable dtHeader, DataSet dsDetail)
        {
        ...
        //Form Events
        private void frmREFIndustry_Load(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;

            try
            {
                this.tagrdIndustry.DisplayLayout.Bands[0].SortedColumns.Clear();
                //Call Initialization
                this.objIndustryFactory = new BOLib.REFIndustryFactory(BOLib.GEnum.InstanceMode.Normal);
                if (objIndustryFactory.GUID <= 0)
                {
                    formClose = true;
                    return;
                }
                ...
                this.objIndustryFactory.ErrorNotifierHeader_Set += new GVar.UINotifierEvent(this.ErrorNotifier_Set);
                this.objIndustryFactory.ErrorNotifierHeader_Clear += new GVar.UINotifierEvent(this.ErrorNotifier_Clear);
                ...
                {
                    if (objIndustryFactory.SetReadOnlyData(_dtHeader, _dsDetail) == false)
                    {
                        formClose = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    else if (formOpenMode == GEnum.formInitMode.Add)
                    {
                        if (canEditRecordID && recordID != string.Empty)
                            this.IndustryID.SetValueTrigger(recordID, false);
                    }
                }

                //Setup Grid Layout, ShortcutMenu, fill Combo
                GlobalUI.FormGrids_Set(this, (int)objIndustryFactory.ConstantCodeKey, out ContextMenuSetting);
                ContextMenuSetting += GlobalUI.ContextMenuSetting_GetNew((int)objIndustryFactory.ConstantCodeKey);
                //GlobalUI.cmnuGlobal_Set(this);
                GlobalUI.Combos_Fill(this, (int)objIndustryFactory.ConstantCodeKey);

                //Setup List properties
                this.tagrdIndustry.DisplayLayout.Override.AllowDelete = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdIndustry.DisplayLayout.Override.AllowUpdate = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdIndustry.DisplayLayout.Bands[0].Override.ActiveCellAppearance.BackColor = Color.Gold;
        ...
        private void frmREFIndustry_Shown(object sender, EventArgs e)
        {
            //Close Form when FormLoad has encounter errors
            if (formClose)
                this.Close();
        }//Completed
        private void frmREFIndustry_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.formClose && objIndustryFactory == null)
                return;
            try
            {
                ...
                //Dispose Factory
                if ((bool)this.objIndustryFactory.Dispose() == false)
            ...
            {
                if (e.Cancel == false)
                    this.objIndustryFactory.Dispose();
            }
        }//Completed
        private void frmREFIndustry_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void Refresh_Header()
        {
            try
            {
                this.bdsIndustryDet.DataSource = objIndustryFactory.ObjREFIndustry;
                this.bdsIndustryDet.AllowNew = true;
                this.bdsIndustryDet.ResetBindings(false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void Refresh_GridList()
        {
            try
            {
                ...
                {
                    string listID = GlobalUI.ListSettingID_Get(ContextMenuSetting, "0GF", tagrdIndustry.Name);
                    GlobalUI.Grid_Format(tagrdIndustry, listID, true, false);
                }
                this.tagrdIndustry.DataSource = BOLib.REFList.GetIndustrys(out msgID);
                if (this.tagrdIndustry.DisplayLayout.Bands[0].SortedColumns.Count <= 0)
                    this.tagrdIndustry.DisplayLayout.Bands[0].SortedColumns.Add(tagrdIndustry.DisplayLayout.Bands[0].Columns["IndustryID"], false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        {
            bool EnableMode = !this.objIndustryFactory.IsReadOnly;
            this.tslReadOnly.Text = !EnableMode ? "Read Only" : string.Empty;

            //Special Condition for RecordID
            this.IndustryID.Enabled = EnableMode;
            this.IndustryID.ReadOnly = !canEditRecordID;

            this.IndustryDes.Enabled = EnableMode;
            ...
            else
            {
                ...
                if (this.objIndustryFactory.IsNew)
                {
        ...
        private void ListSelectionSync()
        {
            ...
            try
            {
                if (objIndustryFactory.ObjREFIndustry.IndustryKey > 0)
                {
                    ...
                    if (tagrdIndustry.Selected != null)
                        if (tagrdIndustry.Selected.Cells.Count > 0)
                            if (GFunc.NEInt(tagrdIndustry.Selected.Cells[0].Row.Cells["IndustryKey"].Value, 0) == objIndustryFactory.ObjREFIndustry.IndustryKey)
                                return;//already selected.

                    //Select the row on the list to reflect the current value in Header.recordkey
                    UltraGridRow ToSelectRow = this.tagrdIndustry.Rows.OfType<UltraGridRow>().ToList().Find(
                    row => row.Cells["IndustryKey"].Text.Equals(objIndustryFactory.ObjREFIndustry.IndustryKey.ToString(), StringComparison.CurrentCultureIgnoreCase));
                    if (ToSelectRow != null)
                    {
                        ToSelectRow.Cells["IndustryID"].Selected = true;
                        ToSelectRow.Cells["IndustryID"].Activate();
                    }
                }
                else
                {
                    tagrdIndustry.Selected.Cells.Clear();
                    tagrdIndustry.ActiveRow = null;
                }
            }
            catch (TAException tex)
            {
        ...
        private bool New_Process()
        {
            ...
            try
            {
                if (form_CanValidate() == false)
                {
                    ...
                    {
                        this.objIndustryFactory.IsDirty = false;
                    }
                }

                this.errorProvider1.Clear();

                if (this.objIndustryFactory.IsDirty)
                {
                    if (Save_Process() == false)
                        return false;
                }

                if (this.objIndustryFactory.New() == false)
                {                  
                    return false;
                }
                else
        ...
        private bool Save_Process()
        {
            ...
            try
            {
                ...
                //Perform Saving
                if (this.objIndustryFactory.Save())
                {
                    GlobalUI.ResetControlDirty(this);
                    return true;
                }
        ...
        private bool OpenRecord(int key)
        {
            ...
            try
            {
                if (SECPermUtility.Edit(objIndustryFactory.PermID, false))
                {
                    if (objIndustryFactory.GetEdit(key) == false)
                    {
                        if (SysOptionUtility.GetBool("WarnOpenRecordAsReadOnly"))
                        {
                            ...
                            {
                                objIndustryFactory.GetReadOnly(key);
                            }
                        }
                    }
                }
        ...
        private bool Delete_Process()
        {
            ...
            try
            {
                ...
                if (this.objIndustryFactory.Delete())
                {
                    this.objIndustryFactory.New();
                   
                    //Move the cursor position of active row index to upper row
                    if (tagrdIndustry.ActiveRow.Index > 0)
                        PreRowIndex = tagrdIndustry.ActiveRow.Index - 1;
            ...
            finally
            {
                this.Refresh_Form(false);
                this.FormLayout();
                if (tagrdIndustry.Rows.Count > 0)
                {
                    tagrdIndustry.Rows[PreRowIndex].Selected = true;
                    tagrdIndustry.Rows[PreRowIndex].Activate();
                }
                this.Cursor = Cursors.Default;
            }
        }//Completed
        private bool Clear_Process()
        {
            ...
            try
            {
                if (GFunc.IsNEZ(this.objIndustryFactory.ObjREFIndustry.IndustryKey))
                {
                    ...
                    if (this.objIndustryFactory.New())
                    {                       
        ...
        //Grid Events
        private void tagrdIndustry_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void tagrdIndustry_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {
            int key = 0;
            try
            {
                ...
                {
                    if (this.tagrdIndustry.ActiveRow != null && tagrdIndustry.Selected.Cells.Count > 0)//this line is required to check if user have selected a row in the list
                        key = GFunc.NEInt(tagrdIndustry.Selected.Cells[0].Row.Cells["IndustryKey"].Value, 0);
