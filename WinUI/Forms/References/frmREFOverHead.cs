...
namespace WinUI
{
    public partial class frmREFOverHead : Form
    {
        #region Local Variables
        private BOLib.REFOverHeadFactory objOverHeadFactory = null;
        ...
        //Initialize
        public frmREFOverHead()
        {
            InitializeComponent();
        }//Completed
        public frmREFOverHead(string id)
        {
            InitializeComponent();
            recordID = id;
        ...
        public frmREFOverHead(GEnum.SystemCode DocCodeKey, DataTable dtHeader, DataSet dsDetail)
        {
        ...
        //Form Events
        private void frmREFOverHead_Load(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;

            try
            {
                this.tagrdOverHead.DisplayLayout.Bands[0].SortedColumns.Clear();
                //Call Initialization
                this.objOverHeadFactory = new BOLib.REFOverHeadFactory(BOLib.GEnum.InstanceMode.Normal);
                if (objOverHeadFactory.GUID <= 0)
                {
                    formClose = true;
                    return;
                }
                ...
                this.objOverHeadFactory.ErrorNotifierHeader_Set += new GVar.UINotifierEvent(this.ErrorNotifier_Set);
                this.objOverHeadFactory.ErrorNotifierHeader_Clear += new GVar.UINotifierEvent(this.ErrorNotifier_Clear);
                ...
                {
                    if (objOverHeadFactory.SetReadOnlyData(_dtHeader, _dsDetail) == false)
                    {
                        formClose = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    else if (formOpenMode == GEnum.formInitMode.Add)
                    {
                        if (canEditRecordID && recordID != string.Empty)
                            this.OverHeadID.SetValueTrigger(recordID, false);
                    }
                }

                //Setup Grid Layout, ShortcutMenu, fill Combo
                GlobalUI.FormGrids_Set(this, (int)objOverHeadFactory.ConstantCodeKey, out ContextMenuSetting);
                ContextMenuSetting += GlobalUI.ContextMenuSetting_GetNew((int)objOverHeadFactory.ConstantCodeKey);
                //GlobalUI.cmnuGlobal_Set(this);
                GlobalUI.Combos_Fill(this, (int)objOverHeadFactory.ConstantCodeKey);

                //Setup List properties
                this.tagrdOverHead.DisplayLayout.Override.AllowDelete = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdOverHead.DisplayLayout.Override.AllowUpdate = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdOverHead.DisplayLayout.Bands[0].Override.ActiveCellAppearance.BackColor = Color.Gold;
        ...
        private void frmREFOverHead_Shown(object sender, EventArgs e)
        {
            //Close Form when FormLoad has encounter errors
            if (formClose)
                this.Close();
        }//Completed
        private void frmREFOverHead_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.formClose && objOverHeadFactory == null)
                return;
            try
            {
                ...
                //Dispose Factory
                if ((bool)this.objOverHeadFactory.Dispose() == false)
            ...
            {
                if (e.Cancel == false)
                    this.objOverHeadFactory.Dispose();
            }
        }//Completed
        private void frmREFOverHead_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void Refresh_Header()
        {
            try
            {
                this.bdsOverHeadDet.DataSource = objOverHeadFactory.ObjREFOverHead;
                this.bdsOverHeadDet.AllowNew = true;
                this.bdsOverHeadDet.ResetBindings(false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void Refresh_GridList()
        {
            try
            {
                ...
                {
                    string listID = GlobalUI.ListSettingID_Get(ContextMenuSetting, "0GF", tagrdOverHead.Name);
                    GlobalUI.Grid_Format(tagrdOverHead, listID, true, false);
                }
                if (this.tagrdOverHead.DisplayLayout.Bands[0].SortedColumns.Count <= 0)
                    this.tagrdOverHead.DisplayLayout.Bands[0].SortedColumns.Add(tagrdOverHead.DisplayLayout.Bands[0].Columns["OverHeadID"], false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        {
            bool EnableMode = !this.objOverHeadFactory.IsReadOnly;
            this.tslReadOnly.Text = !EnableMode ? "Read Only" : string.Empty;

            //Special Condition for RecordID
            this.OverHeadID.Enabled = EnableMode;
            this.OverHeadID.ReadOnly = !canEditRecordID;

            this.OverHeadCost.Enabled = EnableMode;
            this.OverHeadDes.Enabled = EnableMode;
            ...
            else
            {
                ...
                if (this.objOverHeadFactory.IsNew)
                {
        ...
        private void ListSelectionSync()
        {
            ...
            try
            {
                if (objOverHeadFactory.ObjREFOverHead.OverHeadKey > 0)
                {
                    ...
                    if (tagrdOverHead.Selected != null)
                        if (tagrdOverHead.Selected.Cells.Count > 0)
                            if (GFunc.NEInt(tagrdOverHead.Selected.Cells[0].Row.Cells["OverHeadKey"].Value, 0) == objOverHeadFactory.ObjREFOverHead.OverHeadKey)
                                return;//already selected.

                    //Select the row on the list to reflect the current value in Header.recordkey
                    UltraGridRow ToSelectRow = this.tagrdOverHead.Rows.OfType<UltraGridRow>().ToList().Find(
                    row => row.Cells["OverHeadKey"].Text.Equals(objOverHeadFactory.ObjREFOverHead.OverHeadKey.ToString(), StringComparison.CurrentCultureIgnoreCase));
                    if (ToSelectRow != null)
                    {
                        ToSelectRow.Cells["OverHeadID"].Selected = true;
                        ToSelectRow.Cells["OverHeadID"].Activate();
                    }
                }
                else
                {
                    tagrdOverHead.Selected.Cells.Clear();
                    tagrdOverHead.ActiveRow = null;
                }
            }
            catch (TAException tex)
            {
        ...
        private bool New_Process()
        {
            ...
            try
            {
                if (form_CanValidate() == false)
                {
                    ...
                    {
                        this.objOverHeadFactory.IsDirty = false;
                    }
                }

                this.errorProvider1.Clear();

                if (this.objOverHeadFactory.IsDirty)
                {
                    if (Save_Process() == false)
                        return false;
                }

                if (this.objOverHeadFactory.New() == false)
                {                   
                    return false;
                }
                else
        ...
        private bool Save_Process()
        {
            ...
            try
            {
                ...
                //Perform Saving
                if (this.objOverHeadFactory.Save())
                {
        ...
        private bool OpenRecord(int key)
        {
            ...
            try
            {
                if (SECPermUtility.Edit(objOverHeadFactory.PermID, false))
                {
                    if (objOverHeadFactory.GetEdit(key) == false)
                    {
                        if (SysOptionUtility.GetBool("WarnOpenRecordAsReadOnly"))
                        {
                            ...
                            {
                                objOverHeadFactory.GetReadOnly(key);
                            }
                        }
                    }
                }
        ...
        private bool Delete_Process()
        {
            ...
            try
            {
                ...
                if (this.objOverHeadFactory.Delete())
                {
                    this.objOverHeadFactory.New();
                    
                    //Move the cursor position of active row index to upper row
                    if (tagrdOverHead.ActiveRow.Index > 0)
                        PreRowIndex = tagrdOverHead.ActiveRow.Index - 1;
                    GlobalUI.ResetControlDirty(this);
                    return true;
                }
                else
            ...
            finally
            {
                ...
                if (tagrdOverHead.Rows.Count > 0)
                {
                    tagrdOverHead.Rows[PreRowIndex].Selected = true;
                    tagrdOverHead.Rows[PreRowIndex].Activate();
                }
                this.Cursor = Cursors.Default;
            }
        }//Completed
        private bool Clear_Process()
        {
            ...
            try
            {
                if (GFunc.IsNEZ(this.objOverHeadFactory.ObjREFOverHead.OverHeadKey))
                {
                    ...
                    if (this.objOverHeadFactory.New())
                    { 
        ...
        //Grid Events
        private void tagrdOverHead_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void tagrdOverHead_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {
            int key = 0;
            try
            {
                ...
                {
                    if (this.tagrdOverHead.ActiveRow != null && tagrdOverHead.Selected.Cells.Count > 0)//this line is required to check if user have selected a row in the list
                        key = GFunc.NEInt(tagrdOverHead.Selected.Cells[0].Row.Cells["OverHeadKey"].Value, 0);
