...
namespace WinUI
{
    public partial class frmREFPackingType : Form
    {
        #region Local Variables
        private BOLib.REFPackingTypeFactory objPackingTypeFactory = null;
        ...
        //Initialize
        public frmREFPackingType()
        {
            InitializeComponent();
        }//Completed
        public frmREFPackingType(string id)
        {
            InitializeComponent();
            recordID = id;
        ...
        public frmREFPackingType(int Key)
        {
        ...
        public frmREFPackingType(GEnum.SystemCode DocCodeKey, DataTable dtHeader, DataSet dsDetail)
        {
        ...
        //Form Events
        private void frmREFPackingType_Load(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;

            try
            {
                this.tagrdPackingType.DisplayLayout.Bands[0].SortedColumns.Clear();
                //Call Initialization
                this.objPackingTypeFactory = new BOLib.REFPackingTypeFactory(BOLib.GEnum.InstanceMode.Normal);
                if (objPackingTypeFactory.GUID <= 0)
                {
                    formClose = true;
                    return;
                }
                ...
                this.objPackingTypeFactory.ErrorNotifierHeader_Set += new GVar.UINotifierEvent(this.ErrorNotifier_Set);
                this.objPackingTypeFactory.ErrorNotifierHeader_Clear += new GVar.UINotifierEvent(this.ErrorNotifier_Clear);
                ...
                {
                    if (objPackingTypeFactory.SetReadOnlyData(_dtHeader, _dsDetail) == false)
                    {
                        formClose = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    else if (formOpenMode == GEnum.formInitMode.Add)
                    {
                        if (canEditRecordID && recordID != string.Empty)
                            this.PackingTypeID.SetValueTrigger(recordID, false);
                    }
                }

                //Setup Grid Layout, ShortcutMenu, fill Combo
                GlobalUI.FormGrids_Set(this, (int)objPackingTypeFactory.ConstantCodeKey, out ContextMenuSetting);
                ContextMenuSetting += GlobalUI.ContextMenuSetting_GetNew((int)objPackingTypeFactory.ConstantCodeKey);
                //GlobalUI.cmnuGlobal_Set(this);
                GlobalUI.Combos_Fill(this, (int)objPackingTypeFactory.ConstantCodeKey);

                //Setup List properties
                this.tagrdPackingType.DisplayLayout.Override.AllowDelete = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdPackingType.DisplayLayout.Override.AllowUpdate = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdPackingType.DisplayLayout.Bands[0].Override.ActiveCellAppearance.BackColor = Color.Gold;
        ...
        private void frmREFPackingType_Shown(object sender, EventArgs e)
        {
            //Close Form when FormLoad has encounter errors
            if (formClose)
                this.Close();
        }//Completed
        private void frmREFPackingType_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.formClose && objPackingTypeFactory == null)
                return;
            try
            {
                ...
                //Dispose Factory
                if ((bool)this.objPackingTypeFactory.Dispose() == false)
            ...
            {
                if (e.Cancel == false)
                    this.objPackingTypeFactory.Dispose();
            }
        }//Completed
        private void frmREFPackingType_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void Refresh_Header()
        {
            try
            {
                this.bdsPackingTypeDet.DataSource = objPackingTypeFactory.ObjREFPackingType;
                this.bdsPackingTypeDet.AllowNew = true;
                this.bdsPackingTypeDet.ResetBindings(false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void Refresh_GridList()
        {
            try
            {
                ...
                {
                    string listID = GlobalUI.ListSettingID_Get(ContextMenuSetting, "0GF", tagrdPackingType.Name);
                    GlobalUI.Grid_Format(tagrdPackingType, listID, true, false);
                }
                if (this.tagrdPackingType.DisplayLayout.Bands[0].SortedColumns.Count <= 0)
                    this.tagrdPackingType.DisplayLayout.Bands[0].SortedColumns.Add(tagrdPackingType.DisplayLayout.Bands[0].Columns["PackingTypeID"], false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        {
            bool EnableMode = !this.objPackingTypeFactory.IsReadOnly;
            this.tslReadOnly.Text = !EnableMode ? "Read Only" : string.Empty;

            //Special Condition for RecordID
            this.PackingTypeID.Enabled = EnableMode;
            this.PackingTypeID.ReadOnly = !canEditRecordID;

            this.PackingTypeDes.Enabled = EnableMode;
            ...
            else
            {
                ...
                if (this.objPackingTypeFactory.IsNew)
                {
        ...
        private void ListSelectionSync()
        {
            ...
            try
            {
                if (objPackingTypeFactory.ObjREFPackingType.PackingTypeKey > 0)
                {
                    ...
                    if (tagrdPackingType.Selected != null)
                        if (tagrdPackingType.Selected.Cells.Count > 0)
                            if (GFunc.NEInt(tagrdPackingType.Selected.Cells[0].Row.Cells["PackingTypeKey"].Value, 0) == objPackingTypeFactory.ObjREFPackingType.PackingTypeKey)
                                return;//already selected.

                    //Select the row on the list to reflect the current value in Header.recordkey
                    UltraGridRow ToSelectRow = this.tagrdPackingType.Rows.OfType<UltraGridRow>().ToList().Find(
                    row => row.Cells["PackingTypeKey"].Text.Equals(objPackingTypeFactory.ObjREFPackingType.PackingTypeKey.ToString(), StringComparison.CurrentCultureIgnoreCase));
                    if (ToSelectRow != null)
                    {
                        ToSelectRow.Cells["PackingTypeID"].Selected = true;
                        ToSelectRow.Cells["PackingTypeID"].Activate();
                    }
                }
                else
                {
                    tagrdPackingType.Selected.Cells.Clear();
                    tagrdPackingType.ActiveRow = null;
                }
            }
            catch (TAException tex)
            {
        ...
        private bool New_Process()
        {
            ...
            try
            {
                if (form_CanValidate() == false)
                {
                    ...
                    {
                        this.objPackingTypeFactory.IsDirty = false;
                    }
                }

                this.errorProvider1.Clear();

                if (this.objPackingTypeFactory.IsDirty)
                {
                    if (Save_Process() == false)
                        return false;
                }

                if (this.objPackingTypeFactory.New() == false)
                {                   
                    return false;
                }
                else
        ...
        private bool SaveChanges()
        {
            try
            {
                ...
                if (objPackingTypeFactory.IsDirty)
                {
                    GEnum.MsgBoxButton btnSelect;
        ...
        private bool Save_Process()
        {
            ...
            try
            {
                ...
                //Perform Saving
                if (this.objPackingTypeFactory.Save())
                {
                    GlobalUI.ResetControlDirty(this);
                    return true;
                }
        ...
        private bool OpenRecord(int key)
        {
            ...
            try
            {
                if (SECPermUtility.Edit(objPackingTypeFactory.PermID, false))
                {
                    if (objPackingTypeFactory.GetEdit(key) == false)
                    {
                        if (SysOptionUtility.GetBool("WarnOpenRecordAsReadOnly"))
                        {
                            ...
                            {
                                objPackingTypeFactory.GetReadOnly(key);
                            }
                        }
                    }
                }
        ...
        private bool Delete_Process()
        {
            ...
            try
            {
                ...
                if (this.objPackingTypeFactory.Delete())
                {
                    this.objPackingTypeFactory.New();
                   
                    //Move the cursor position of active row index to upper row
                    if (tagrdPackingType.ActiveRow.Index > 0)
                        PreRowIndex = tagrdPackingType.ActiveRow.Index - 1;
            ...
            finally
            {
                this.Refresh_Form(false);
                this.FormLayout();
                if (tagrdPackingType.Rows.Count > 0)
                {
                    tagrdPackingType.Rows[PreRowIndex].Selected = true;
                    tagrdPackingType.Rows[PreRowIndex].Activate();
                }
                this.Cursor = Cursors.Default;
            }
        }//Completed
        private bool Clear_Process()
        {
            ...
            try
            {
                if (GFunc.IsNEZ(this.objPackingTypeFactory.ObjREFPackingType.PackingTypeKey))
                {
                    ...
                    if (this.objPackingTypeFactory.New())
                    {                       
        ...
        //Grid Events
        private void tagrdPackingType_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void tagrdPackingType_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {
            int key = 0;
            try
            {
                ...
                {
                    if (this.tagrdPackingType.ActiveRow != null && tagrdPackingType.Selected.Cells.Count > 0)//this line is required to check if user have selected a row in the list
                        key = GFunc.NEInt(tagrdPackingType.Selected.Cells[0].Row.Cells["PackingTypeKey"].Value, 0);
