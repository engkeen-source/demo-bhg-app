...
namespace WinUI
{
    public partial class frmREFScale : Form
    {
        #region Local Variables
        private BOLib.REFScaleFactory objScaleFactory = null;
        ...
        public frmREFScale(GEnum.SystemCode DocCodeKey, DataTable dtHeader, DataSet dsDetail)
        {
        ...
        //Form Events
        private void frmREFScale_Load(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;

            try
            {
                this.tagrdScaleList.DisplayLayout.Bands[0].SortedColumns.Clear();
                //Call Initialization
                this.objScaleFactory = new BOLib.REFScaleFactory(BOLib.GEnum.InstanceMode.Normal);
                if (objScaleFactory.GUID <= 0)
                {
                    formClose = true;
                    return;
                }
                ...
                this.objScaleFactory.ErrorNotifierHeader_Set += new GVar.UINotifierEvent(this.ErrorNotifier_Set);
                this.objScaleFactory.ErrorNotifierHeader_Clear += new GVar.UINotifierEvent(this.ErrorNotifier_Clear);
                ...
                {
                    if (objScaleFactory.SetReadOnlyData(_dtHeader, _dsDetail) == false)
                    {
                        formClose = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    else if (formOpenMode == GEnum.formInitMode.Add)
                    {
                        if (canEditRecordID && recordID != string.Empty)
                            this.ScaleID.SetValueTrigger(recordID, false);
                    }
                }

                //Setup Grid Layout, ShortcutMenu, fill Combo
                GlobalUI.FormGrids_Set(this, (int)objScaleFactory.ConstantCodeKey, out ContextMenuSetting);
                ContextMenuSetting += GlobalUI.ContextMenuSetting_GetNew((int)objScaleFactory.ConstantCodeKey);
                //GlobalUI.cmnuGlobal_Set(this);
                GlobalUI.Combos_Fill(this, (int)objScaleFactory.ConstantCodeKey);

                //Setup List properties
                this.tagrdScaleList.DisplayLayout.Override.AllowDelete = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdScaleList.DisplayLayout.Override.AllowUpdate = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdScaleList.DisplayLayout.Bands[0].Override.ActiveCellAppearance.BackColor = Color.Gold;
        ...
        private void frmREFScale_Shown(object sender, EventArgs e)
        {
            //Close Form when FormLoad has encounter errors
            if (formClose)
                this.Close();
        }//Completed
        private void frmREFScale_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.formClose && objScaleFactory == null)
                return;
            ...
            try
            {
                ...
                //Dispose Factory
                if ((bool)this.objScaleFactory.Dispose() == false)
            ...
            {
                if (e.Cancel == false)
                    this.objScaleFactory.Dispose();
            }
        }//Completed
        private void frmREFScale_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void Refresh_Header()
        {
            try
            {
                this.bdsScaleDet.DataSource = objScaleFactory.ObjREFScale;
                this.bdsScaleDet.AllowNew = true;
                this.bdsScaleDet.ResetBindings(false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void Refresh_GridList()
        {
            try
            {
                ...
                {
                    string listID = GlobalUI.ListSettingID_Get(ContextMenuSetting, "0GF", tagrdScaleList.Name);
                    GlobalUI.Grid_Format(tagrdScaleList, listID, true, false);
                }
                if (this.tagrdScaleList.DisplayLayout.Bands[0].SortedColumns.Count <= 0)
                    this.tagrdScaleList.DisplayLayout.Bands[0].SortedColumns.Add(tagrdScaleList.DisplayLayout.Bands[0].Columns["ScaleID"], false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        {
            bool EnableMode = !this.objScaleFactory.IsReadOnly;
            this.tslReadOnly.Text = !EnableMode ? "Read Only" : string.Empty;

            //Special Condition for RecordID
            this.ScaleID.Enabled = EnableMode;
            this.ScaleID.ReadOnly = !canEditRecordID;

            this.ScaleDes.Enabled = EnableMode;

            this.Size1.Enabled = EnableMode;
            this.Size2.Enabled = EnableMode;
            this.Size3.Enabled = EnableMode;
            this.Size4.Enabled = EnableMode;
            this.Size5.Enabled = EnableMode;
            this.Size6.Enabled = EnableMode;
            this.Size7.Enabled = EnableMode;
            this.Size8.Enabled = EnableMode;
            this.Size9.Enabled = EnableMode;
            this.Size10.Enabled = EnableMode;
            this.Size11.Enabled = EnableMode;
            this.Size12.Enabled = EnableMode;
            this.Size13.Enabled = EnableMode;
            this.Size14.Enabled = EnableMode;
            this.Size15.Enabled = EnableMode;
            this.Size16.Enabled = EnableMode;
            this.Size17.Enabled = EnableMode;
            this.Size18.Enabled = EnableMode;
            this.Size19.Enabled = EnableMode;
            this.Size20.Enabled = EnableMode;
            this.Size21.Enabled = EnableMode;
            this.Size22.Enabled = EnableMode;
            this.Size23.Enabled = EnableMode;
            this.Size24.Enabled = EnableMode;
            this.Size25.Enabled = EnableMode;
            this.Size26.Enabled = EnableMode;
            this.Size27.Enabled = EnableMode;
            this.Size28.Enabled = EnableMode;
            this.Size29.Enabled = EnableMode;
            this.Size30.Enabled = EnableMode;
        ...
        private void ListSelectionSync()
        {
            ...
            try
            {
                if (objScaleFactory.ObjREFScale.ScaleKey > 0)
                {
                    ...
                    if (tagrdScaleList.Selected != null)
                        if (tagrdScaleList.Selected.Cells.Count > 0)
                            if (GFunc.NEInt(tagrdScaleList.Selected.Cells[0].Row.Cells["ScaleKey"].Value, 0) == objScaleFactory.ObjREFScale.ScaleKey)
                                return;//already selected.

                    //Select the row on the list to reflect the current value in Header.recordkey
                    UltraGridRow ToSelectRow = this.tagrdScaleList.Rows.OfType<UltraGridRow>().ToList().Find(
                    row => row.Cells["ScaleKey"].Text.Equals(objScaleFactory.ObjREFScale.ScaleKey.ToString(), StringComparison.CurrentCultureIgnoreCase));
                    if (ToSelectRow != null)
                    {
                        ToSelectRow.Cells["ScaleID"].Selected = true;
                        ToSelectRow.Cells["ScaleID"].Activate();
                    }
                }
                else
                {
                    tagrdScaleList.Selected.Cells.Clear();
                    tagrdScaleList.ActiveRow = null;
                }
            }
            catch (TAException tex)
            {
        ...
        private bool New_Process()
        {
            ...
            try
            {
                if (form_CanValidate() == false)
                {
                    ...
                    {
                        this.objScaleFactory.IsDirty = false;
                    }
                }

                this.errorProvider1.Clear();

                if (this.objScaleFactory.IsDirty)
                {
                    if (Save_Process() == false)
                        return false;
                }

                if (this.objScaleFactory.New() == false)
                {                    
                    return false;
                }
                else
        ...
        private bool OpenRecord(int key)
        {
            ...
            try
            {
                if (SECPermUtility.Edit(objScaleFactory.PermID, false))
                {
                    if (objScaleFactory.GetEdit(key) == false)
                    {
                        if (SysOptionUtility.GetBool("WarnOpenRecordAsReadOnly"))
                        {
                            ...
                            {
                                objScaleFactory.GetReadOnly(key);
                            }
                        }
                    }
                }
        ...
        private bool Delete_Process()
        {
            ...
            try
            {
                ...
                if (this.objScaleFactory.Delete())
                {
                    this.objScaleFactory.New();
                   
                    //Move the cursor position of active row index to upper row
                    if (tagrdScaleList.ActiveRow.Index > 0)
                        PreRowIndex = tagrdScaleList.ActiveRow.Index - 1;
            ...
            finally
            {
                ...
                if (tagrdScaleList.Rows.Count > 0)
                {
                    tagrdScaleList.Rows[PreRowIndex].Selected = true;
                    tagrdScaleList.Rows[PreRowIndex].Activate();
                }
        ...
        private bool Clear_Process()
        {
            ...
            try
            {
                if (GFunc.IsNEZ(this.objScaleFactory.ObjREFScale.ScaleKey))
                {
        ...
        //Grid Events
        private void tagrdScale_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void tagrdScale_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {
            int key = 0;
            try
            {
                ...
                {
                    if (this.tagrdScaleList.ActiveRow != null && tagrdScaleList.Selected.Cells.Count > 0)//this line is required to check if user have selected a row in the list
                        key = GFunc.NEInt(tagrdScaleList.Selected.Cells[0].Row.Cells["ScaleKey"].Value, 0);
