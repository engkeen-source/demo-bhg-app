...
namespace WinUI
{
    public partial class frmREFShipVia : Form
    {
        #region Local Variables
        private BOLib.REFShipViaFactory objShipViaFactory = null;
        ...
        //Initialize
        public frmREFShipVia()
        {
            InitializeComponent();
        }//Completed
        public frmREFShipVia(string id)
        {
            InitializeComponent();
            recordID = id;
        ...
        public frmREFShipVia(GEnum.SystemCode DocCodeKey, DataTable dtHeader, DataSet dsDetail)
        {
        ...
        //Form Events
        private void frmREFShipVia_Load(object sender, EventArgs e)
        {
            ...
            {
                //Call Initialization
                this.objShipViaFactory = new BOLib.REFShipViaFactory(BOLib.GEnum.InstanceMode.Normal);
                if (objShipViaFactory.GUID <= 0)
                {
                    formClose = true;
                    return;
                }
                ...
                this.objShipViaFactory.ErrorNotifierHeader_Set += new GVar.UINotifierEvent(this.ErrorNotifier_Set);
                this.objShipViaFactory.ErrorNotifierHeader_Clear += new GVar.UINotifierEvent(this.ErrorNotifier_Clear);
                ...
                {
                    if (objShipViaFactory.SetReadOnlyData(_dtHeader, _dsDetail) == false)
                    {
                        formClose = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    else if (formOpenMode == GEnum.formInitMode.Add)
                    {
                        if (canEditRecordID && recordID != string.Empty)
                            this.ShipViaID.SetValueTrigger(recordID, false);
                    }
                }

                //Setup Grid Layout, ShortcutMenu, fill Combo
                GlobalUI.FormGrids_Set(this, (int)objShipViaFactory.ConstantCodeKey, out ContextMenuSetting);
                ContextMenuSetting += GlobalUI.ContextMenuSetting_GetNew((int)objShipViaFactory.ConstantCodeKey);
                //GlobalUI.cmnuGlobal_Set(this);
                GlobalUI.Combos_Fill(this, (int)objShipViaFactory.ConstantCodeKey);

                //Setup List properties
                this.tagrdShipVia.DisplayLayout.Override.AllowDelete = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdShipVia.DisplayLayout.Override.AllowUpdate = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdShipVia.DisplayLayout.Bands[0].Override.ActiveCellAppearance.BackColor = Color.Gold;
        ...
        private void frmREFShipVia_Shown(object sender, EventArgs e)
        {
            //Close Form when FormLoad has encounter errors
            if (formClose)
                this.Close();
        }//Completed
        private void frmREFShipVia_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.formClose && objShipViaFactory == null)
                return;
            ...
            try
            {
                ...
                //Dispose Factory
                if ((bool)this.objShipViaFactory.Dispose() == false)
            ...
            {
                if (e.Cancel == false)
                    this.objShipViaFactory.Dispose();
            }
        }//Completed
        private void frmREFShipVia_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void Refresh_Header()
        {
            try
            {
                this.bdsShipViaDet.DataSource = objShipViaFactory.ObjREFShipVia;
                this.bdsShipViaDet.AllowNew = true;
                this.bdsShipViaDet.ResetBindings(false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void Refresh_GridList()
        {
            try
            {
                ...
                {
                    string listID = GlobalUI.ListSettingID_Get(ContextMenuSetting, "0GF", tagrdShipVia.Name);
                    GlobalUI.Grid_Format(tagrdShipVia, listID, true, false);
                }
                if (this.tagrdShipVia.DisplayLayout.Bands[0].SortedColumns.Count <= 0)
                    this.tagrdShipVia.DisplayLayout.Bands[0].SortedColumns.Add(tagrdShipVia.DisplayLayout.Bands[0].Columns["ShipViaID"], false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        {
            bool EnableMode = !this.objShipViaFactory.IsReadOnly;
            this.tslReadOnly.Text = !EnableMode ? "Read Only" : string.Empty;

            //Special Condition for RecordID
            this.ShipViaID.Enabled = EnableMode;
            this.ShipViaID.ReadOnly = !canEditRecordID;

            this.ShipViaDes.Enabled = EnableMode;
            ...
            else
            {
                ...
                if (this.objShipViaFactory.IsNew)
                {
        ...
        private void ListSelectionSync()
        {
            ...
            try
            {
                if (objShipViaFactory.ObjREFShipVia.ShipViaKey > 0)
                {
                    ...
                    if (tagrdShipVia.Selected != null)
                        if (tagrdShipVia.Selected.Cells.Count > 0)
                            if (GFunc.NEInt(tagrdShipVia.Selected.Cells[0].Row.Cells["ShipViaKey"].Value, 0) == objShipViaFactory.ObjREFShipVia.ShipViaKey)
                                return;//already selected.

                    //Select the row on the list to reflect the current value in Header.recordkey
                    UltraGridRow ToSelectRow = this.tagrdShipVia.Rows.OfType<UltraGridRow>().ToList().Find(
                    row => row.Cells["ShipViaKey"].Text.Equals(objShipViaFactory.ObjREFShipVia.ShipViaKey.ToString(), StringComparison.CurrentCultureIgnoreCase));
                    if (ToSelectRow != null)
                    {
                        ToSelectRow.Cells["ShipViaID"].Selected = true;
                        ToSelectRow.Cells["ShipViaID"].Activate();
                    }
                }
                else
                {
                    tagrdShipVia.Selected.Cells.Clear();
                    tagrdShipVia.ActiveRow = null;
                }
            }
            catch (TAException tex)
            {
        ...
        private bool New_Process()
        {
            ...
            try
            {
                if (form_CanValidate() == false)
                {
                    ...
                    {
                        this.objShipViaFactory.IsDirty = false;
                    }
                }

                this.errorProvider1.Clear();

                if (this.objShipViaFactory.IsDirty)
                {
                    if (Save_Process() == false)
                        return false;
                }

                if (this.objShipViaFactory.New() == false)
                {
                    return false;
                }
                else
        ...
        private bool Save_Process()
        {
            ...
            try
            {
                ...
                //Perform Saving
                if (this.objShipViaFactory.Save())
                {
                    GlobalUI.ResetControlDirty(this);
                    return true;
                }
        ...
        private bool OpenRecord(int key)
        {
            ...
            try
            {
                if (SECPermUtility.Edit(objShipViaFactory.PermID, false))
                {
                    if (objShipViaFactory.GetEdit(key) == false)
                    {
                        if (SysOptionUtility.GetBool("WarnOpenRecordAsReadOnly"))
                        {
                            ...
                            {
                                objShipViaFactory.GetReadOnly(key);
                            }
                        }
                    }
                }
        ...
        private bool Delete_Process()
        {
            ...
            try
            {
                ...
                if (this.objShipViaFactory.Delete())
                {
                    this.objShipViaFactory.New();
                   
                    //Move the cursor position of active row index to upper row
                    if (tagrdShipVia.ActiveRow.Index > 0)
                        PreRowIndex = tagrdShipVia.ActiveRow.Index - 1;
            ...
            finally
            {
                this.Refresh_Form(false);
                this.FormLayout();
                if (tagrdShipVia.Rows.Count > 0)
                {
                    tagrdShipVia.Rows[PreRowIndex].Selected = true;
                    tagrdShipVia.Rows[PreRowIndex].Activate();
                }
                this.Cursor = Cursors.Default;
            }
        }//Completed
        private bool Clear_Process()
        {
            ...
            try
            {
                if (GFunc.IsNEZ(this.objShipViaFactory.ObjREFShipVia.ShipViaKey))
                {
        ...
        //Grid Events
        private void tagrdShipVia_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void tagrdShipVia_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {
            int key = 0;
            try
            {
                ...
                {
                    if (this.tagrdShipVia.ActiveRow != null && tagrdShipVia.Selected.Cells.Count > 0)//this line is required to check if user have selected a row in the list
                        key = GFunc.NEInt(tagrdShipVia.Selected.Cells[0].Row.Cells["ShipViaKey"].Value, 0);
