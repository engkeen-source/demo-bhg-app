...
namespace WinUI
{
    public partial class frmREFWorkOrderReqType : Form
    {
         #region Local Variables
        private BOLib.REFWorkOrderRequestTypeFactory objWorkOrderReqTypeFactory = null;
        ...
        //Initialize
        public frmREFWorkOrderReqType()
        {
            InitializeComponent();
        }//Completed
        public frmREFWorkOrderReqType(string id)
        {
            InitializeComponent();
            recordID = id;
        ...
        public frmREFWorkOrderReqType(int Key)
        {
        ...
        public frmREFWorkOrderReqType(GEnum.SystemCode DocCodeKey, DataTable dtHeader, DataSet dsDetail)
        {
        ...
        //Form Events
        private void frmREFWorkOrderReqType_Load(object sender, EventArgs e)
        {
            this.Cursor = Cursors.WaitCursor;

            try
            {
                this.tagrdReqType.DisplayLayout.Bands[0].SortedColumns.Clear();
                //Call Initialization
                this.objWorkOrderReqTypeFactory = new BOLib.REFWorkOrderRequestTypeFactory(BOLib.GEnum.InstanceMode.Normal);
                if (objWorkOrderReqTypeFactory.GUID <= 0)
                {
                    formClose = true;
                    return;
                }
                ...
                this.objWorkOrderReqTypeFactory.ErrorNotifierHeader_Set += new GVar.UINotifierEvent(this.ErrorNotifier_Set);
                this.objWorkOrderReqTypeFactory.ErrorNotifierHeader_Clear += new GVar.UINotifierEvent(this.ErrorNotifier_Clear);
                ...
                {
                    if (objWorkOrderReqTypeFactory.SetReadOnlyData(_dtHeader, _dsDetail) == false)
                    {
                        formClose = true;
                        return;
                    }
                ...
                else
                {
                    ...
                    else if (formOpenMode == GEnum.formInitMode.Add)
                    {
                        if (canEditRecordID && recordID != string.Empty)
                            this.ReqTypeID.SetValueTrigger(recordID, false);
                    }
                }

                //Setup Grid Layout, ShortcutMenu, fill Combo
                GlobalUI.FormGrids_Set(this, (int)objWorkOrderReqTypeFactory.ConstantCodeKey, out ContextMenuSetting);
                ContextMenuSetting += GlobalUI.ContextMenuSetting_GetNew((int)objWorkOrderReqTypeFactory.ConstantCodeKey);
                //GlobalUI.cmnuGlobal_Set(this);
                GlobalUI.Combos_Fill(this, (int)objWorkOrderReqTypeFactory.ConstantCodeKey);

                //Setup List properties
                this.tagrdReqType.DisplayLayout.Override.AllowDelete = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdReqType.DisplayLayout.Override.AllowUpdate = Infragistics.Win.DefaultableBoolean.False;
                this.tagrdReqType.DisplayLayout.Bands[0].Override.ActiveCellAppearance.BackColor = Color.Gold;
        ...
        private void frmREFWorkOrderReqType_Shown(object sender, EventArgs e)
        {
            //Close Form when FormLoad has encounter errors
            if (formClose)
                this.Close();
        }//Completed
        private void frmREFWorkOrderReqType_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (this.formClose && objWorkOrderReqTypeFactory == null)
                return;
            try
            {
                ...
                //Dispose Factory
                if ((bool)this.objWorkOrderReqTypeFactory.Dispose() == false)
            ...
            {
                if (e.Cancel == false)
                    this.objWorkOrderReqTypeFactory.Dispose();
            }
        }//Completed
        private void frmREFWorkOrderReqType_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
        ...
        private void Refresh_Header()
        {
            try
            {
                this.bdsReqTypeDet.DataSource = objWorkOrderReqTypeFactory.ObjREFWorkOrderReqType;
                this.bdsReqTypeDet.AllowNew = true;
                this.bdsReqTypeDet.ResetBindings(false);
            }
            catch (TAException tex)
            {
                throw Error(tex, false);
        ...
        private void Refresh_GridList()
        {
            try
            {
                ...
                {
                    string listID = GlobalUI.ListSettingID_Get(ContextMenuSetting, "0GF", tagrdReqType.Name);
                    GlobalUI.Grid_Format(tagrdReqType, listID, true, false);
                }
                this.tagrdReqType.DataSource = BOLib.REFList.GetWORequestTypes(out msgID);
                if (this.tagrdReqType.DisplayLayout.Bands[0].SortedColumns.Count <= 0)
                    this.tagrdReqType.DisplayLayout.Bands[0].SortedColumns.Add(tagrdReqType.DisplayLayout.Bands[0].Columns["ReqTypeID"], false);
        ...
        {
            bool EnableMode = !this.objWorkOrderReqTypeFactory.IsReadOnly;
            this.tslReadOnly.Text = !EnableMode ? "Read Only" : string.Empty;

            //Special Condition for RecordID
            this.ReqTypeID.Enabled = EnableMode;
            this.ReqTypeID.ReadOnly = !canEditRecordID;

            this.ReqTypeDes.Enabled = EnableMode;
            this.DefItemKey.Enabled = EnableMode;
            ...
            else
            {
                ...
                if (this.objWorkOrderReqTypeFactory.IsNew)
                {
        ...
        private void ListSelectionSync()
        {
            ...
            try
            {
                if (objWorkOrderReqTypeFactory.ObjREFWorkOrderReqType.ReqTypeKey > 0)
                {
                    ...
                    if (tagrdReqType.Selected != null)
                        if (tagrdReqType.Selected.Cells.Count > 0)
                            if (GFunc.NEInt(tagrdReqType.Selected.Cells[0].Row.Cells["ReqTypeKey"].Value, 0) == objWorkOrderReqTypeFactory.ObjREFWorkOrderReqType.ReqTypeKey)
                                return;//already selected.

                    //Select the row on the list to reflect the current value in Header.recordkey
                    UltraGridRow ToSelectRow = this.tagrdReqType.Rows.OfType<UltraGridRow>().ToList().Find(
                    row => row.Cells["ReqTypeKey"].Text.Equals(objWorkOrderReqTypeFactory.ObjREFWorkOrderReqType.ReqTypeKey.ToString(), StringComparison.CurrentCultureIgnoreCase));
                    if (ToSelectRow != null)
                    {
                        ToSelectRow.Cells["ReqTypeID"].Selected = true;
                        ToSelectRow.Cells["ReqTypeID"].Activate();
                    }
                }
                else
                {
                    tagrdReqType.Selected.Cells.Clear();
                    tagrdReqType.ActiveRow = null;
                }
            }
            catch (TAException tex)
            {
        ...
        private bool New_Process()
        {
            ...
            try
            {
                if (form_CanValidate() == false)
                {
                    ...
                    {
                        this.objWorkOrderReqTypeFactory.IsDirty = false;
                    }
                }

                this.errorProvider1.Clear();

                if (this.objWorkOrderReqTypeFactory.IsDirty)
                {
                    if (Save_Process() == false)
                        return false;
                }

                if (this.objWorkOrderReqTypeFactory.New() == false)
                {
                    return false;
                }
                else
        ...
        private bool SaveChanges()
        {
            try
            {
                ...
                if (objWorkOrderReqTypeFactory.IsDirty)
                {
                    GEnum.MsgBoxButton btnSelect;
        ...
        private bool Save_Process()
        {
            ...
            try
            {
                ...
                //Perform Saving
                if (this.objWorkOrderReqTypeFactory.Save())
                {
                    GlobalUI.ResetControlDirty(this);
                    return true;
                }
        ...
        private bool OpenRecord(int key)
        {
            ...
            try
            {
                if (SECPermUtility.Edit(objWorkOrderReqTypeFactory.PermID, false))
                {
                    if (objWorkOrderReqTypeFactory.GetEdit(key) == false)
                    {
                        if (SysOptionUtility.GetBool("WarnOpenRecordAsReadOnly"))
                        {
                            ...
                            {
                                objWorkOrderReqTypeFactory.GetReadOnly(key);
                            }
                        }
                    }
                }
        ...
        private bool Delete_Process()
        {
            ...
            try
            {
                ...
                if (this.objWorkOrderReqTypeFactory.Delete())
                {
                    this.objWorkOrderReqTypeFactory.New();

                    //Move the cursor position of active row index to upper row
                    if (tagrdReqType.ActiveRow.Index > 0)
                        PreRowIndex = tagrdReqType.ActiveRow.Index - 1;
            ...
            finally
            {
                ...
                if (tagrdReqType.Rows.Count > 0)
                {
                    tagrdReqType.Rows[PreRowIndex].Selected = true;
                    tagrdReqType.Rows[PreRowIndex].Activate();
                }
                this.Cursor = Cursors.Default;
            }
        }//Completed
        private bool Clear_Process()
        {
            ...
            try
            {
                if (GFunc.IsNEZ(this.objWorkOrderReqTypeFactory.ObjREFWorkOrderReqType.ReqTypeKey))
                {
                    ...
                    if (this.objWorkOrderReqTypeFactory.New())
                    {
        ...
        //Grid Events
        private void tagrdReqType_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void tagrdReqType_AfterSelectChange(object sender, AfterSelectChangeEventArgs e)
        {
            int key = 0;
            try
            {
                ...
                {
                    if (this.tagrdReqType.ActiveRow != null && tagrdReqType.Selected.Cells.Count > 0)//this line is required to check if user have selected a row in the list
                        key = GFunc.NEInt(tagrdReqType.Selected.Cells[0].Row.Cells["ReqTypeKey"].Value, 0);
